...
- **2026-06-04 (Codex)**: T-1248 knowledge-dashboard browser QA and Korean copy guard hardening. Playwright CLI browser QA against the local Knowledge Dashboard confirmed API-key login, operations/readiness rendering, knowledge, QA/QC, and activity tab clicks, `/api/auth/session` plus all four data routes returning 200, console warnings/errors 0, and `/api/data/readiness` reporting score 94 with only the existing Hanwoo T-251 blocker. `projects/knowledge-dashboard/src/encoding-guard.test.mts` now rejects common Korean mojibake fragments across source files and asserts core operations/readiness Korean copy remains readable in `app/page.tsx` and `ProductReadinessPanel.tsx`. `.gitignore` now ignores `.playwright-cli/` and `output/playwright/` so browser QA screenshots/snapshots do not pollute future git status. Verification passed Knowledge Dashboard project QC (`61` tests plus lint/build), `git diff --check`, A/B `adopt_candidate` (`score_delta=0.4666666666666667`), and completion audit `complete`. A separate untracked `projects/knowledge-dashboard/knowledge-t1248-browser.spec.ts` from the parallel browser QA run was preserved outside this commit scope.
