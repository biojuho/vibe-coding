...
- `workspace`: cleanup PR `#25` now points to branch `chore/public-repo-cleanup`, and the remote branch itself is at `9c296a9` after CI-unblock follow-up commits. As of the last check in this session, GitHub PR metadata was still lagging on the previous head `ebfaec9`, so stale failed checks in the PR UI may not reflect the latest pushed branch state yet.
...
- GitHub PR metadata lag was observed after pushing `9c296a9` to `chore/public-repo-cleanup`: `git ls-remote` shows the remote head updated, but `gh pr view 25` was still reporting the previous head `ebfaec9` and its older failed checks. Re-check the PR before assuming the latest commits were ignored.
