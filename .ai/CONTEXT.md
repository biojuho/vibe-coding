...
- Focused `blind-to-x` test runs for those new modules pass (`4 passed` and `10 passed`), but they currently surface a non-blocking Pydantic V1 `@validator` deprecation warning from `projects/blind-to-x/pipeline/models.py`.
- `projects/blind-to-x/pipeline/observability.py::_is_async()` currently emits a Python deprecation warning because it still uses `asyncio.iscoroutinefunction`; move it to `inspect.iscoroutinefunction` before Python 3.16.
