...
- `T-155` landed on `2026-04-06`: `projects/hanwoo-dashboard/src/lib/action-validation.mjs` now provides shared server-side payload validation/normalization for the riskiest mutation paths, and `src/lib/actions.js` routes `createCattle`, `updateCattle`, `createSalesRecord`, `recordFeed`, `addInventoryItem`, `updateInventoryQuantity`, `updateFarmSettings`, and `createExpenseRecord` through those guards before Prisma writes. Focused unit coverage now lives in `src/lib/action-validation.test.mjs`, and `package.json` exposes `npm test` for that suite (`9 passed` on `2026-04-06`).
- `T-156` landed on `2026-04-07`: `projects/hanwoo-dashboard/src/lib/payment-confirmation.mjs` now safely reads Toss confirmation responses without assuming valid JSON, classifies retryable upstream failures (`timeout`, network error, `5xx`, malformed or empty `200` body) as `pending`, and keeps definitive rejections (`4xx`, amount mismatch) as explicit failures. `src/app/api/payments/confirm/route.js` now uses that helper, only marks `PaymentLog` as `FAILED` for definitive cases, and returns `500` for unexpected internal exceptions. Focused unit coverage lives in `src/lib/payment-confirmation.test.mjs`, and `npm test` now runs both local `.mjs` suites (`17 passed` on `2026-04-07`).
- `T-157` landed on `2026-04-07`: `projects/hanwoo-dashboard/src/lib/market-price-state.mjs` now defines the authoritative market-price states (`kape-live`, `kape-cache`, `cache-stale`, `unavailable`) and rejects legacy non-authoritative snapshots (`isRealtime: false`) so previously-simulated rows do not get reused as if they were live production data. The KAPE client no longer manufactures sample prices when the upstream is unavailable, `getRealTimeMarketPrice()` only persists trusted live payloads, and `src/components/widgets/MarketPriceWidget.js` now surfaces explicit source labels instead of collapsing everything into a misleading live/sample badge. Focused unit coverage lives in `src/lib/market-price-state.test.mjs`, and `npm test` now runs 23 local tests (`23 passed` on `2026-04-07`).
- `T-158` landed on `2026-04-07`: `projects/hanwoo-dashboard/src/lib/cattle-history.mjs` now parses cattle-history metadata row-by-row so one malformed JSON cell no longer poisons the entire result set. `src/lib/actions.js#getCattleHistory` now returns normalized rows with `metadataParseError`/`metadataRaw` for damaged entries, and `src/components/forms/CattleDetailModal.js` now consumes normalized metadata directly instead of trying to `JSON.parse()` already-parsed history entries. Focused unit coverage lives in `src/lib/cattle-history.test.mjs`, and `npm test` now runs 27 local tests (`27 passed` on `2026-04-07`).
- `T-159` landed on `2026-04-07`: `projects/hanwoo-dashboard/src/lib/weather-state.mjs` now defines the defensive weather parsing/normalization path for Open-Meteo payloads, including explicit `available`, `degraded`, and `isStale` states plus safe `unavailable`, `stale`, and `partial forecast` messaging. `src/components/DashboardClient.js` now fetches weather through `fetchWithTimeout`, degrades cleanly on timeout/non-OK/malformed responses, and preserves the last good snapshot instead of silently leaving the widget in a broken state. Focused unit coverage lives in `src/lib/weather-state.test.mjs`, and `npm test` now runs 34 local tests (`34 passed` on `2026-04-07`).
- Minefield: this Node build does not accept `--experimental-default-type=module`, and `hanwoo-dashboard` still has no package-wide `"type": "module"`. For project-local Node unit tests that import live source modules, prefer `.mjs` helpers/tests (for example `src/lib/action-validation.mjs` + `src/lib/action-validation.test.mjs`) instead of assuming `.js` ESM imports will run directly under `node --test`.
