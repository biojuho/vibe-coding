...
- `projects/knowledge-dashboard/src/app/page.tsx` now validates authenticated route payload shapes before storing dashboard state, keeps QA/QC route failures non-fatal, separates auth errors from generic data-load failures in the UI, and keeps the dashboard bearer key in memory instead of persisting it in `localStorage`. `src/lib/dashboard-insights.ts` also treats large `Unspecified` language buckets as metadata gaps instead of a dominant stack.
- Latest `knowledge-dashboard` verification on `2026-04-02`: `python -m py_compile scripts/sync_data.py`, `npm run lint`, and `npm run build` all pass.
