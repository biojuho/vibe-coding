## Rotation 2026-05-26 (archived addenda older than 2026-05-25)
...
| Date | 2026-05-21 |
| Tool | Codex |
| Work | **T-625 completed**: Hanwoo cattle and sales pagination hooks now normalize page item payloads. `useCattlePagination()` and `useSalesPagination()` route initial SSR items and load-more API `json.data.items` through `normalizePaginationItems()`, ignore non-array input and malformed/non-object rows, and append from safe arrays so malformed pagination/cache payloads cannot crash through spread access. Code commit `676f7453`. |
| Next Priorities | Verification passed: focused pagination source tests (`4 passed`), targeted ESLint passed, path-limited `git diff --check` passed, full Hanwoo QC passed (`test` 263, lint, build), and staged `code_review_gate --json` emitted JSON pass (`risk_score 0.0`; cp949 reader-thread noise only). Commit hook WARN was the known graph/test-gap heuristic. Active Hanwoo goal remains open; T-251 remains external/user-owned Supabase control-plane resync, and T-372/T-407 remain approval-scoped. Preserve unrelated WIP, especially separate `CattleDetailModal`, monorepo, and shorts-maker changes. |
...
| Date | 2026-05-21 |
| Tool | Codex |
| Work | **T-608 completed**: Hanwoo cattle form now normalizes building payloads before rendering. `CattleForm` routes incoming `buildings` props through `normalizeCattleFormBuildings()`, ignores non-array/non-object malformed payloads, supplies stable fallback ids plus Korean missing-building copy, and renders/reset/defaults from `safeBuildings` so stale cache or caller data cannot crash the form through raw `.map()` or value derivation. Code commit `d76f1058`. |
| Next Priorities | Verification passed: focused cattle detail/form wiring test (`11 passed`), targeted ESLint passed, path-limited `git diff --check` passed, full Hanwoo QC passed (`test` 249, lint, build), and staged `code_review_gate --json` passed (`risk_score 0.0`; cp949 reader-thread noise only). Commit hook WARN was the known graph/test-gap heuristic and saw unrelated WIP. Active Hanwoo goal remains open; T-251 remains external/user-owned Supabase control-plane resync, and T-372/T-407 remain approval-scoped. Preserve unrelated root/Hanwoo package/ScheduleTab/shorts/workspace WIP. |
...
| Date | 2026-05-21 |
| Tool | Codex |
| Work | **T-605 completed**: Hanwoo feed tab now normalizes feed-related payloads before rendering. `FeedTab` routes incoming `cattle`, `feedStandards`, `feedHistory`, and `buildings` props through safe collection normalizers, ignores non-array/non-object malformed payloads, supplies stable fallback building/record ids plus Korean missing-building copy, and renders from `safeCattle`, `safeFeedStandards`, `safeFeedHistory`, and `safeBuildings` so stale cache/caller data cannot crash feed summaries, filters, charts, or recent records through raw `.filter()`/`.forEach()`/spread/slice access. Code commit `186a031f`. |
| Next Priorities | Verification passed: focused empty-state/feed wiring test (`15 passed`), targeted ESLint passed, path-limited `git diff --check` passed, full Hanwoo QC passed (`test` 247, lint, build), and staged `code_review_gate --json` passed (`risk_score 0.0`; cp949 reader-thread noise only). Commit hook WARN was the known graph/test-gap heuristic and saw unrelated WIP. Active Hanwoo goal remains open; T-251 remains external/user-owned Supabase control-plane resync, and T-372/T-407 remain approval-scoped. Preserve unrelated root/Hanwoo package/shorts/workspace WIP. |
...
| Date | 2026-05-21 |
| Tool | Codex |
| Work | **T-590 completed**: Hanwoo cattle form cancel action now announces its save-in-flight state. `CattleForm` already disabled the bottom cancel button while `isSaving`; it now derives `cancelButtonLabel` and applies `aria-busy`, `aria-label`, and `title` so assistive technology and hover text explain that cancellation is unavailable during cattle save. Code commit `977956b8`. |
| Next Priorities | Verification passed: focused cattle detail/form wiring test (`10 passed`), targeted ESLint passed, path-limited `git diff --check`, full `project_qc_runner --project hanwoo-dashboard --json` passed (`test` 240, lint, build), and staged `code_review_gate --json` passed (`risk_score 0.0`; cp949 reader-thread noise only). Commit hook WARN was the known graph/test-gap heuristic while direct tests and full QC covered the changed files. Active Hanwoo goal remains open; T-251 remains external/user-owned Supabase control-plane resync, and T-372/T-407 remain approval-scoped. Preserve unrelated root/Hanwoo package/widgets/shorts/workspace WIP. |

| Field | Value |
|---|---|
| Date | 2026-05-21 |
| Tool | Codex |
| Work | **T-589 completed**: Hanwoo Schedule tab event dates now reject impossible calendar inputs before calendar grouping, upcoming-list filtering, sorting, and D-day labels. `ScheduleTab` round-trips `YYYY-MM-DD` date keys in `toValidDate()`, so impossible schedule dates such as `2026-02-31` can no longer roll forward through JavaScript `Date` parsing into false March calendar/upcoming entries. Code commit `1dccff1e`. |
| Next Priorities | Verification passed: focused tab-header accessibility/source test (`6 passed`), targeted ESLint passed, path-limited `git diff --check`, full `project_qc_runner --project hanwoo-dashboard --json` passed (`test` 240, lint, build), and staged `code_review_gate --json` passed (`risk_score 0.0`; cp949 reader-thread noise only). Commit hook WARN was the known graph/test-gap heuristic while direct tests and full QC covered the changed files. Active Hanwoo goal remains open; T-251 remains external/user-owned Supabase control-plane resync, and T-372/T-407 remain approval-scoped. Preserve unrelated root/Hanwoo package/shorts/workspace WIP. |
...
| Date | 2026-05-21 |
| Tool | Codex |
| Work | **T-581 completed**: Hanwoo AI chat send button now announces its busy state while the assistant response is streaming. `AIChatWidget` already disabled duplicate sends through `canSend` / `sendInFlightRef`; the send icon button now also exposes `aria-busy={isStreaming}` so assistive technology receives the same pending state as the visible spinner/title. Code commit `e05a92be`. |
...
| Date | 2026-05-21 |
| Tool | Codex |
| Work | **T-580 completed**: Hanwoo QR label print button now shows print-preparation copy while a print window is being prepared. `QRCodeWidget` already blocked duplicate print windows with `isPrinting`; the button now also changes its accessible label, title, and visible text to `QR 라벨 인쇄 준비 중` / `인쇄 준비 중...` during that busy state. Code commit `1eb569b3`. |
...
| Date | 2026-05-21 |
| Tool | Codex |
| Work | **T-579 completed**: Hanwoo notification modal now stays locked while the SMS test send is in flight. `NotificationModal` already disabled the test-send action; it now also guards Escape dismissal, overlay click dismissal, and the close button while `isTestingSMS` is true, with close button `aria-busy` so operators do not lose send state mid-flight. Code commit `f7f23d4d`. |
...
| Date | 2026-05-21 |
| Tool | Codex |
| Work | **T-576 completed**: Hanwoo cattle detail modal now stays locked while archive/delete or breeding-record save work is in flight. `CattleDetailModal` ignores Escape dismissal during `isDeleting` / `isBreedingSaving`, and the top close/back icon is disabled with `aria-busy`, preventing destructive or record-saving flows from being dismissed through an alternate path. Code commit `9db2090b`. |
| Next Priorities | Verification passed: focused cattle detail/form wiring test (`10 passed`), targeted ESLint passed, path-limited `git diff --check`, full `project_qc_runner --project hanwoo-dashboard --json` passed (`test` 240, lint, build), and staged `code_review_gate --json` passed (`risk_score 0.0`; cp949 reader-thread noise only). Commit hook WARN was the known graph/test-gap heuristic while direct tests and full QC covered the changed files. Active Hanwoo goal remains open; T-251 remains external/user-owned Supabase control-plane resync, and T-372/T-407 remain approval-scoped. Preserve unrelated root/Hanwoo package/financial-chart/shorts/workspace WIP. |
...
| Date | 2026-05-21 |
| Tool | Codex |
| Work | **T-575 completed**: Hanwoo cattle form now stays locked while a save is in flight. `CattleForm` already disabled the bottom cancel/submit controls during save; it now also ignores Escape dismissal and disables the top back icon with `aria-busy` while saving, preventing an in-flight cattle create/update from being dismissed through an alternate path. Code commit `14ad83cf`. |
| Next Priorities | Verification passed: focused cattle detail/form wiring test (`10 passed`), targeted ESLint passed, path-limited `git diff --check`, full `project_qc_runner --project hanwoo-dashboard --json` passed (`test` 240, lint, build), and staged `code_review_gate --json` passed (`risk_score 0.0`; cp949 reader-thread noise only). Commit hook WARN was the known graph/test-gap heuristic while direct tests and full QC covered the changed files. Active Hanwoo goal remains open; T-251 remains external/user-owned Supabase control-plane resync, and T-372/T-407 remain approval-scoped. Preserve unrelated root/Hanwoo package/financial-chart/shorts/workspace WIP. |
...
| Date | 2026-05-21 |
| Tool | Codex |
| Work | **T-574 completed**: Hanwoo cattle tag lookup now ignores stale MTRACE responses after the cattle form switches record context or unmounts. `CattleForm` tracks lookup request ids plus mounted state, clears lookup loading on form reset, and only writes lookup results for the current request. Code commit `b621bf3b`. |
| Next Priorities | Verification passed: focused cattle detail/form wiring test (`10 passed`), targeted ESLint passed, path-limited `git diff --check`, full `project_qc_runner --project hanwoo-dashboard --json` passed (`test` 240, lint, build), and staged `code_review_gate --json` passed (`risk_score 0.0`; cp949 reader-thread noise only). Commit hook WARN was the known graph/test-gap heuristic while direct tests and full QC covered the changed files. Active Hanwoo goal remains open; T-251 remains external/user-owned Supabase control-plane resync, and T-372/T-407 remain approval-scoped. Preserve unrelated root/Hanwoo package/financial-chart/shorts/workspace WIP. |
...
| Date | 2026-05-21 |
| Tool | Codex |
| Work | **T-573 completed**: Hanwoo weather timeout copy now uses shared Korean product copy instead of the inline English `Weather lookup timed out after 5000ms.` message. `weather-state.mjs` exports `WEATHER_TIMEOUT_MESSAGE`, and both `DashboardClient` plus `useWeather` pass that constant into `fetchWithTimeout`. Code commit `75e22762`. |
| Next Priorities | Verification passed: focused weather/home copy tests (`34 passed`), targeted ESLint passed, path-limited `git diff --check`, full `project_qc_runner --project hanwoo-dashboard --json` passed (`test` 240, lint, build), and staged `code_review_gate --json` passed (`risk_score 0.0`; cp949 reader-thread noise only). Commit hook WARN was the known graph/test-gap heuristic while direct tests and full QC covered the changed files. Active Hanwoo goal remains open; T-251 remains external/user-owned Supabase control-plane resync, and T-372/T-407 remain approval-scoped. Preserve unrelated root/Hanwoo package/financial-chart/shorts/workspace WIP. |
...
| Date | 2026-05-21 |
| Tool | Codex |
| Work | **T-564 completed**: Hanwoo `DashboardClient` now routes fallback monthly sales count dates through `toValidCalendarDate()`. Impossible sale-date strings such as `2026-02-31` can no longer roll forward through JavaScript `Date` parsing and inflate the home monthly 출하 count. Code commit `82a855dd`. |
...
| Date | 2026-05-21 |
| Tool | Codex |
| Work | **T-563 completed**: Hanwoo `MarketPriceWidget` now normalizes incoming price snapshots so missing `bull` or `cow` side objects become empty objects before rendering. Partial cached/live market data can no longer crash the home price widget. Code commit `0c3247f9`. |
...
| Date | 2026-05-21 |
| Tool | Codex |
| Work | **T-562 completed**: Hanwoo dashboard list pagination now builds next-page cursor sort values through `toCursorSortValue()` and closes pagination gracefully when the last item has a malformed sort date. Corrupted `updatedAt`/`saleDate` values can no longer throw through raw `new Date(lastItem[sortField]).toISOString()` and turn cattle/sales list pagination into a 500. Code commit `a76a1898`. |
| Next Priorities | Verification passed: focused dashboard list cursor date source test (`1 passed`), targeted ESLint passed, path-limited `git diff --check`, full `project_qc_runner --project hanwoo-dashboard --json` passed (`test` 236, lint, build), and staged `code_review_gate --json` passed (`risk_score 0.0`; cp949 reader-thread noise only). Commit hook WARN was the known graph/test-gap heuristic while direct tests and full QC covered the changed files. Active Hanwoo goal remains open; T-251 remains external/user-owned Supabase control-plane resync, and T-372/T-407 remain approval-scoped. Preserve unrelated root/Hanwoo package/shorts/workspace WIP. |
...
| Date | 2026-05-21 |
| Tool | Codex |
| Work | **T-559 completed**: Hanwoo `CattleDetailModal` now parses 발정/수정 record dates through strict `YYYY-MM-DD` round-trip validation before saving `lastEstrus` or `pregnancyDate`. Impossible dates such as `2026-02-31` can no longer roll forward through JavaScript `Date` parsing into false breeding records. Code commit `cf93524d`. |
...
| Date | 2026-05-21 |
| Tool | Codex |
| Work | **T-556 completed**: Hanwoo subscription renewal date math now normalizes `addDays()` inputs to a valid base date and finite day count before calculating `nextPaymentDate`. Malformed payment approval dates or day counts can no longer produce `Invalid Date` subscription renewals. Code commit `a5935a57`. |
| Next Priorities | Verification passed: focused subscription date test (`2 passed`), targeted ESLint passed, path-limited `git diff --check`, full `project_qc_runner --project hanwoo-dashboard --json` passed (`test` 233, lint, build), and staged `code_review_gate --json` passed (`risk_score 0.0`; cp949 reader-thread noise only). Commit hook WARN was the known graph/test-gap heuristic while direct tests and full QC covered the changed files. Active Hanwoo goal remains open; T-251 remains external/user-owned Supabase control-plane resync, and T-372/T-407 remain approval-scoped. Preserve unrelated root/Hanwoo package/expense/shorts/workspace WIP. |
...
| Date | 2026-05-21 |
| Tool | Codex |
| Work | **T-553 completed**: Hanwoo market price state now round-trips strict `YYYYMMDD`, `YYYY.MM.DD`, and `YYYY-MM-DD` KAPE issue-date inputs before accepting them. Impossible dates such as `20260231` now fall back to the current snapshot date instead of rolling forward to a false display date. Code commit `6812782c`. |
| Next Priorities | Verification passed: focused market-price state test (`7 passed`), targeted ESLint passed, path-limited `git diff --check`, full `project_qc_runner --project hanwoo-dashboard --json` passed (`test` 228, lint, build), and staged `code_review_gate --json` passed (`risk_score 0.0`; cp949 reader-thread noise only). Commit hook WARN was the known graph/test-gap heuristic while direct tests and full QC covered the changed files. Active Hanwoo goal remains open; T-251 remains external/user-owned Supabase control-plane resync, and T-372/T-407 remain approval-scoped. Preserve unrelated root/Hanwoo package/shorts/workspace WIP. |
...
| Date | 2026-05-21 |
| Tool | Codex |
| Work | **T-548 completed**: Hanwoo cattle CSV export now blanks malformed birth dates through `formatCsvDate()`, preventing browser-default `Invalid Date` text from reaching exported cattle CSV deliverables. Code commit `b901931d`. |
| Next Priorities | Verification passed: focused cattle CSV tests (`3 passed`), targeted ESLint passed, path-limited `git diff --check`, full `project_qc_runner --project hanwoo-dashboard --json` passed (`test` 223, lint, build), and staged `code_review_gate --json` passed (`risk_score 0.0`; cp949 reader-thread noise only). Commit hook WARN was the known graph/test-gap heuristic while direct tests and full QC covered the changed files. Active Hanwoo goal remains open; T-251 remains external/user-owned Supabase control-plane resync, and T-372/T-407 remain approval-scoped. Preserve unrelated root/Hanwoo package/shorts/workspace WIP. |
...
| Date | 2026-05-21 |
| Tool | Codex |
| Work | **T-536 completed**: Hanwoo dashboard fallback average cattle weight now normalizes cattle weights before aggregation. `DashboardClient` uses `toFiniteNumber(cow.weight)`, preventing malformed/non-finite values from spreading `NaN` into the home dashboard when SSR summary data is unavailable. Code commit `fd18e6eb`. |
| Next Priorities | Verification passed: focused home/market source test (`24 passed`), targeted ESLint passed, path-limited `git diff --check`, full `project_qc_runner --project hanwoo-dashboard --json` passed (`test` 216, lint, build), and staged `code_review_gate --json` passed (`risk_score 0.0`; cp949 reader-thread noise only). Commit hook WARN was the known graph/test-gap heuristic while direct tests and full QC covered the changed files. Active Hanwoo goal remains open; T-251 remains external/user-owned Supabase control-plane resync, and T-372/T-407 remain approval-scoped. Preserve unrelated root/Hanwoo package/Hanwoo FeedTab-empty-state/shorts/suika/workspace WIP. |
...
| Date | 2026-05-21 |
| Tool | Codex |
| Work | **T-529 completed**: Hanwoo Settings farm and building saves now reject duplicate async submits before React re-renders the saving state. `SettingsTab` adds `farmSaveInFlightRef` around `onUpdateFarmSettings()` and `buildingSaveInFlightRef` around `onCreateBuilding()`, while preserving the existing disabled/`aria-busy` UI and location-select lock. Code commit `af4ebf04`. |
| Next Priorities | Verification passed: focused settings accessibility test (`10 passed`), path-limited `git diff --check` passed, full `project_qc_runner --project hanwoo-dashboard --json` passed (`test` 211, lint, build), and `code_review_gate --base HEAD~1 --json` passed (`risk_score 0.0`; cp949 reader-thread noise only). Commit hook WARN was the known graph/test-gap heuristic while direct tests and full QC covered the changed files. Active Hanwoo goal remains open; T-251 remains external/user-owned Supabase control-plane resync, and T-372/T-407 remain approval-scoped. Preserve unrelated root/Hanwoo package/shorts/suika/workspace WIP. |
