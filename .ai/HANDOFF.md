...
| Work | **아키텍처 비평 기반 즉시 수정 5건 + QC 완료.** (1) `blind-to-x` T-128 테스트 격리: `conftest.py`의 `clear_runtime_state` 픽스처를 `monkeypatch + tmp_path` 기반으로 교체해 공유 SQLite 경로 오염 차단. (2) Python 버전 명시: `workspace`, `blind-to-x`, `shorts-maker-v2` 세 `pyproject.toml` 모두 `>=3.10` → `>=3.11`. (3) `next-auth` beta 리스크 문서화: `hanwoo-dashboard/package.json`에 `engines` 필드 추가, `.ai/DECISIONS.md`에 ADR-024 기록. (4) SQLite 8개 DB → `workspace.db` 단일 파일 통합: `execution/` 8개 모듈 `DB_PATH` 수정, `health_check.py` `DB_CHECKS` 정리, `pipeline_watchdog.py` 경로 수정, MCP 레지스트리 하위 호환 유지, 마이그레이션 스크립트(`migrate_to_workspace_db.py`) 신규 작성. (5) `blind-to-x` `cost_db` PostgreSQL 이중화 제거: `cost_db_pg.py`(776줄) + `test_cost_db_pg.py` 삭제, `get_cost_db()` SQLite 단일 경로로 단순화. QC 결과: **933 passed / 0 failed** (blind-to-x), **27 passed** (pipeline_watchdog), 컴파일 11개 전 통과. `test_notification.py` mock lambda `**_` 추가로 기존 실패 1건도 수정. |
