...
| T-323 | `[hanwoo-dashboard]` Added App Router error boundaries for product completeness. The app had no `error.js`/`not-found.js`/`global-error.js`, so runtime errors and unknown URLs fell through to Next.js default debug screens. Added three branded status pages reusing login design tokens: a server-component 404 with a route home link, a client route error boundary with retry (`reset()`) + home actions, and a root-layout `global-error` boundary with inlined styles (globals.css is not applied once the root layout is replaced). Added a `Status Pages` CSS block and a source-wiring test mirroring the empty-state pattern. Staged only the own 44-line CSS hunk via `git apply --cached`, preserving a parallel tool's uncommitted `Setup Progress Panel` WIP. Verification: `npm test` 84 passed/0 fail, `npm run lint` pass, `npm run build` pass (`/_not-found` prerendered static). commit `b56592e`. | Claude Code (Opus 4.7 1M) | 2026-05-20 |
| T-323 | `[hanwoo-dashboard]` Added a home-screen Farm Setup / 운영 준비도 panel for first-run product completeness. New deterministic helper `buildSetupProgressItems()` tracks 농장 기본 정보, 축사 구조, 개체 등록, 재고 기준, and 첫 일정, then the dashboard renders progress, completion state, and direct navigation/actions. Also fixed the home empty 축사 CTA so it opens Settings, where 축사 creation actually lives, instead of opening the cattle modal. Verification: Hanwoo `npm.cmd test` `84 passed`, `npm.cmd run lint` passed, `npm.cmd run build` passed, `PYTHONUTF8=1 python -m code_review_graph detect-changes --repo projects/hanwoo-dashboard --base HEAD --brief` risk `0.00`, `git diff --check` passed, dev server `/login` returned `200`, and `/manifest.json` returned `application/json`. | Codex | 2026-05-20 |
