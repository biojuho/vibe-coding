...
| T-318 | `[shorts-maker-v2]` Phase 3 잔여 품질 개선. Duration target/QC 분리는 T-321에서 완료됐고, hook_score<0.6 성공 출하 차단은 T-327에서 완료됐으며, 50.4MB 파일 크기 경계 hold는 T-331에서 완료됐고, scene_qc strict retry 라우팅은 T-334에서 완료됨. 남은 후보: 채널별 TTS 속도/voice role 미세조정. | Any | Medium | safe | 2026-05-19 |
...
| T-332 | `[hanwoo-dashboard]` Localized subscription checkout/result states for product polish. `PaymentWidget` now shows Korean checkout title, loading, preparing, amount button, timeout, and fallback errors instead of English placeholder copy. Subscription success/fail pages now avoid bare `Loading...`, `Processing...`, `Payment confirmed`, and `Code:` strings, replacing them with Korean status/fallback copy. Added source wiring coverage for checkout/result copy. Verification: Hanwoo tests `88 passed`, `npm.cmd run lint` passed, `npm.cmd run build` passed, direct Hanwoo graph risk `0.00`, staged code-review gate PASS before commit; commit hook emitted advisory WARN from graph heuristics. Commit `8937eb1`. | Codex | 2026-05-20 |
