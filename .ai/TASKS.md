...
|---|---|---|
| T-353 | `[hanwoo-dashboard]` Localized cattle tag lookup (MTRACE) fallback copy. `lookupCattleByTag()` now returns Korean operator-facing messages for missing API key, invalid tag number, rate limits, upstream failures, unreadable responses, no-match results, timeouts, and generic lookup errors; the default breed fallback is now `한우` instead of `Hanwoo`. Added mocked MTRACE behavior coverage in `mtrace.test.mjs`. Verification: focused Hanwoo tests passed (`106 passed`), targeted ESLint passed, full `project_qc_runner --project hanwoo-dashboard --json` passed (`test` 106 passed, lint passed, build passed), `git diff --check` passed, English MTRACE fallback scan passed, and UTF-8 graph risk `0.00`. | Codex | 2026-05-20 |
| T-352 | `[hanwoo-dashboard]` Localized dashboard API fallback/error copy. `DashboardClient` now uses Korean timeout/failure messages for dashboard list loads, Korean console diagnostics for summary/notification/list refresh failures, and a Korean footer rights line. `/api/dashboard/{summary,cattle,sales}` now return Korean default 500 fallback messages when the app-authored fallback path is used. Verification: focused Hanwoo home/import tests passed (`103 passed`), targeted ESLint passed, full `project_qc_runner --project hanwoo-dashboard --json` passed (`test` 103 passed, lint passed, build passed), `git diff --check` passed, and UTF-8 graph risk `0.00`. | Codex | 2026-05-20 |
| T-351 | `[hanwoo-dashboard]` Localized the printed cattle QR label footer. The QR print action already used `QR 출력` / `QR 라벨 인쇄`; the printed tag footer now uses `Joolife 한우 스마트팜` instead of `Joolife Smart Farm`, and `qr-widget-copy.test.mjs` guards against the English footer returning. Verification: focused Hanwoo QR/import tests passed (`102 passed`), targeted ESLint passed, full `project_qc_runner --project hanwoo-dashboard --json` passed (`test` 102 passed, lint passed, build passed), `git diff --check` passed, staged code-review gate PASS, and UTF-8 graph risk `0.00`. | Codex | 2026-05-20 |
