...
|---|---|---|---|---|
| T-396 | `[workspace]` Dependabot PR 백로그 21건 정리. 안전 13건(`#56 #57 #58 #59 #61 #62 #66 #67 #69 #73 #74 #75 #76` — patch/minor·range 확장)은 `@dependabot squash and merge`로 드레인 중. 위험 8건(`#60` anthropic 0.43→0.103, `#63` vite-plugin-react 6, `#64` lucide-react 1 — CI 깨짐 확인, `#65` pytest-asyncio 1, `#68` typescript 6, `#70`/`#72` react major, `#71` recharts 3)은 메이저 점프라 별도 검토 보류. | Claude | 2026-05-20 | 안전군은 실 CI 그린 확인 후 머지. `#66/#70/#72`의 `dependabot` 메타체크 실패는 플래키 메타액션이며 실 테스트는 통과. |
...
| T-395 | `[hanwoo-dashboard]` Kept operational create forms open when async submit handlers fail. Sales, Inventory, Schedule, and Settings create forms now await their save handler and only close/reset after a truthy saved result, preserving typed values for retry on failed mutations. `empty-state-wiring.test.mjs` guards the contract. Verification: focused Hanwoo tests passed (`131 passed`), targeted ESLint passed, full Hanwoo QC passed (`test` 131, lint, build), path-limited `git diff --check` passed, and staged code-review gate PASS; commit hook WARN was heuristic. Commit `bf2d363`. | Codex | 2026-05-20 |
