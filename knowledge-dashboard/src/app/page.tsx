...
import { Book, Code, Github, ExternalLink, RefreshCw, Smartphone, Search, FileText, PieChart, Layers } from "lucide-react";
...
// Types based on our python bridged data
...
import DashboardCharts from "../components/DashboardCharts"; // Import Chart Component
...
export default function Dashboard() {
  ...
  const getTags = (item: GithubRepo | Notebook) => {
    ...
    const tags = [];
    if (text.includes('analysis') || text.includes('report') || text.includes('study') || text.includes('연구')) tags.push({ label: '연구 (Research)', color: 'bg-indigo-500/20 text-indigo-300' });
    if (text.includes('project') || text.includes('app') || text.includes('web') || text.includes('dev') || text.includes('코딩')) tags.push({ label: '개발 (Dev)', color: 'bg-emerald-500/20 text-emerald-300' });
    if (text.includes('meeting') || text.includes('plan') || text.includes('회의') || text.includes('기획')) tags.push({ label: '기획 (Plan)', color: 'bg-orange-500/20 text-orange-300' });
    if (text.includes('paper') || text.includes('pdf') || text.includes('논문')) tags.push({ label: '자료 (Docs)', color: 'bg-sky-500/20 text-sky-300' });
  ...
  return (
    <div className="min-h-screen bg-[#0f172a] text-white p-8 relative overflow-hidden font-sans">
          ...
          <>
            ...
            {stats && (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                ...
                {/* Stats Card 3: Knowledge Sources */}
                <Card className="bg-linear-to-br from-slate-900 via-slate-900 to-slate-800">
                  <CardContent className="p-5 flex items-center gap-4">
      ...
      {/* Details Modal via shadcn Dialog */}
