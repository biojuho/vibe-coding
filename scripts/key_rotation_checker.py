...
Usage:
    python scripts/key_rotation_checker.py           # check all keys
    python scripts/key_rotation_checker.py --dry-run  # print only, no Telegram
    python scripts/key_rotation_checker.py --days 60  # custom threshold
"""
...
def format_report(alerts: list[dict], threshold_days: int) -> str:
    ...
    for a in alerts:
        lines.append(
            f"  {a['key']}: {a['age_days']}d old "
            f"(rotated {a['last_rotated']}, {a['overdue_days']}d overdue)"
        )
...
def main() -> None:
    ...
    parser.add_argument("--dry-run", action="store_true",
                        help="Print report only, don't send Telegram")
    parser.add_argument("--update", type=str, metavar="KEY_NAME",
                        help="Mark a key as rotated today")
