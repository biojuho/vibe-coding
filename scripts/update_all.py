...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Root directory to scan (default: workspace root).",
    )
    parser.add_argument(
        "--strategy",
...
def main() -> int:
    ...
    print(
        f"\nStarting updates (strategy={args.strategy}, "
        f"skip_dirty={skip_dirty}, jobs={max(args.jobs, 1)})"
    )

    results: list[RepoUpdateResult] = []
    with ThreadPoolExecutor(max_workers=max(args.jobs, 1)) as executor:
        future_map = {
            executor.submit(update_repo, repo, args.strategy, skip_dirty): repo for repo in repos
        }
