...
def default_items(root: Path, include_env: bool) -> list[BackupItem]:
    items = [
        BackupItem(
            name="word_chain_db",
            source=root / "projects" / "word-chain-pygame" / "word_chain.db",
    ...
    if include_env:
        items.append(
            BackupItem(
                name="personal_agent_env",
                source=root / "projects" / "personal-agent" / ".env",
                target_name="personal_agent.env",
                required=False,
            )
        )
    return items
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--with-env",
        action="store_true",
        help="Include personal-agent .env in backup.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
