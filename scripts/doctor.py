...
PA_ENV = WORKSPACE / "projects" / "personal-agent" / ".env"
...
def _check_env_files(results: List[CheckResult]) -> Dict[str, str]:
    ...
    pa_env = _read_env_file(PA_ENV)
    merged_env = {**root_env, **pa_env}

    if ROOT_ENV.exists() or PA_ENV.exists():
        results.append(
            CheckResult(
                "PASS",
                ".env Files",
                f"Detected env files: root={ROOT_ENV.exists()}, personal-agent={PA_ENV.exists()}",
            )
        )
    else:
        results.append(
            CheckResult(
                "FAIL",
                ".env Files",
                "No .env file found in workspace root or personal-agent.",
