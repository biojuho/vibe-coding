...
class SpaceScaleTemplate(BaseTemplate):
    ...
    def build_scenes(self, data: dict[str, Any]) -> list[Scene]:
        scenes = []
        scenes.append(Scene(
            role="hook", text=data.get("hook_text", "이 스케일, 상상이 되시나요?"),
            duration=4.0,
        ))
        for comp in data.get("comparisons", []):
            ...
            scenes.append(Scene(
                role="body", text=text,
                keywords=comp.get("keywords", []), duration=5.0,
            ))
        for fact in data.get("facts", []):
            if isinstance(fact, dict):
                scenes.append(Scene(
                    role="body", text=fact.get("text", ""),
                    keywords=fact.get("keywords", []), duration=5.0,
                ))
            else:
