...
class HistoryMysteryGenerator:
    ...
    def __init__(self, title: str, background_story: str,
        ...
        self._embers = [
            {"x": random.randint(0, self.W), "y": random.randint(0, self.H),
             "r": random.uniform(1, 3), "vy": random.uniform(-20, -8),
             "vx": random.uniform(-5, 5),
             "a": random.randint(40, 120), "ph": random.uniform(0, math.tau)}
            for _ in range(12)
        ]
    ...
    @staticmethod
    def _eo(t): return 1 - (1 - min(1, max(0, t))) ** 3  # ease-out
    @staticmethod
    def _ei(t): return min(1, max(0, t)) ** 2  # ease-in
    ...
    def _ph1(self, draw, t):
        ...
        if shown:
            ...
            draw.text(((self.W - hw) // 2, 700), shown,
                      font=self.f_hook, fill=(*self.CREAM, 200))
        ...
        if t > 2.0:
            ...
            for tl_i, tl in enumerate(self._title_lines):
                ...
                for gx, gy, ga in [(0, 0, ta), (-1, -1, ta // 4), (1, 1, ta // 4),
                                   (-2, 0, ta // 6), (2, 0, ta // 6)]:
                    draw.text((tx + gx, ty + gy), tl,
                              font=self.f_title, fill=(*self.CRIMSON, ga))
    ...
    def _ph2(self, draw, t):
        ...
        if lt > 0.3:
            ...
            label = "▎배경"
            draw.text((card_x + 30, card_y + 25), label,
                      font=self.f_small, fill=(*self.GOLD, la))
        ...
        for i, line in enumerate(self._story_lines[:16]):
            ...
            col = self.CREAM
            draw.text((card_x + 35, base_y + i * lh), line,
                      font=self.f_body, fill=(*col, la))
    ...
    def _ph3(self, draw, t):
        ...
        for i, myst in enumerate(self.mysteries):
            ...
            for j, line in enumerate(lines):
                draw.text((cx + 85, cy + 25 + j * lh), line,
                          font=self.f_body, fill=(*self.CREAM, alpha))
    ...
    def _ph4(self, draw, t):
        ...
        if lt < 2.5:
            ...
            draw.text(((self.W - tw_) // 2, 600), txt,
                      font=self.f_hook, fill=(*self.GOLD, ta))
        ...
        if lt > 1.5:
            ...
            for i, line in enumerate(self._hypo_lines):
                ...
                if la < 0: la = 0
                draw.text((card_x + 30, card_y + 35 + i * lh), line,
                          font=self.f_body, fill=(*self.CREAM, la))
        ...
        if lt > cta_start:
            ...
            draw.text(((self.W - tw_) // 2, self.H - 400), self.cta,
                      font=self.f_hook, fill=(*self.AMBER, ca))
            ...
            if lt > cta_start + 0.8:
                ...
                draw.text(((self.W - pw) // 2, self.H - 340), p2,
                          font=self.f_small, fill=(*self.CREAM, pa))
