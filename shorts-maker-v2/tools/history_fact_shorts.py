...
class HistoryFactGenerator:
    ...
    def __init__(self, title: str,
        ...
        random.seed(88)
        self._embers = [
            {"x": random.randint(0, self.W), "y": random.randint(self.H // 2, self.H),
             "vx": random.uniform(-8, 8), "vy": random.uniform(-25, -8),
             "r": random.uniform(1, 3), "a": random.randint(30, 90),
    ...
    def _draw_card(self, draw, x, y, w, h, alpha=180, color=None):
        ...
        draw.rounded_rectangle([(x, y), (x + w, y + h)],
                               radius=8, outline=(*self.GOLD, 50), width=1)

    def _draw_badge(self, draw, x, y, text, color=None):
        c = color or self.GOLD
        tw_ = self._tw(text, self.f_badge) + 20
        draw.rounded_rectangle([(x, y), (x + tw_, y + 32)],
                               radius=4, fill=(*c, 50), outline=(*c, 100), width=1)
    ...
    def _draw_separator(self, draw, y, width=400):
        """장식 구분선."""
        cx = self.W // 2
        draw.line([(cx - width // 2, y), (cx + width // 2, y)],
                  fill=(*self.GOLD, 60), width=1)
    ...
    def _ph1(self, draw, t):
        ...
        if t > 0.5:
            # Era badge
            if self.era:
                ea = int(180 * self._eo((t - 0.5) / 0.6))
                self._draw_badge(draw, self.MG, 560, f"📜 {self.era}", self.GOLD)
            if self.region:
                ra = int(180 * self._eo((t - 0.8) / 0.6))
                rw = self._draw_badge(draw, self.MG + 200, 560, f"📍 {self.region}", self.GOLD)
        ...
        if t > 1.0:
            ...
            if shown:
                ...
                draw.text(((self.W - hw) // 2, 650), shown,
                          font=self.f_big, fill=(*self.CRIMSON, ha))
        ...
        if t > 2.8:
            ...
            for i, line in enumerate(self._title_lines):
                ...
                draw.text(((self.W - tw_) // 2, 780 + i * lh), line,
                          font=self.f_title, fill=(*self.IVORY, la))
    ...
    def _ph2(self, draw, t):
        ...
        if lt > 0.8 and self._myth_lines:
            ...
            for i, line in enumerate(self._myth_lines):
                ...
                if la < 0: la = 0
                draw.text((cx + 30, cy + 30 + i * lh), line,
                          font=self.f_body, fill=(*self.IVORY, la))

        # Era & region (subtle background)
        if self.era and lt > 1.5:
            ea = int(50 * self._eo((lt - 1.5) / 0.5))
            draw.text((self.MG, self.H - 200), self.era,
                      font=self.f_era, fill=(*self.GOLD, ea))
    ...
    def _ph3(self, draw, t):
        ...
        if lt < 3.0:
            ...
            for dx, dy in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
                draw.text(((self.W - tw_) // 2 + dx, self.H // 2 - 60 + dy), txt,
                          font=self.f_big, fill=(*self.CRIMSON, ta // 3))
            draw.text(((self.W - tw_) // 2, self.H // 2 - 60), txt,
                      font=self.f_big, fill=(*self.CRIMSON, ta))
        ...
        if lt > 2.0 and self._fact_lines:
            ...
            for i, line in enumerate(self._fact_lines):
                ...
                if la < 0: la = 0
                draw.text((cx + 30, cy + 50 + i * lh), line,
                          font=self.f_body, fill=(*self.IVORY, la))
        ...
        if lt > 5.0 and self.details:
            ...
            for i, det_lines in enumerate(self._detail_lines):
                ...
                # Bullet
                draw.text((self.MG + 20, base_y + i * 90), "▸",
                          font=self.f_body, fill=(*self.GOLD, al))
                for j, ln in enumerate(det_lines):
                    draw.text((self.MG + 50, base_y + i * 90 + j * lh), ln,
                              font=self.f_body, fill=(*self.IVORY, al))
    ...
    def _ph4(self, draw, t):
        ...
        if self._lesson_lines and lt > 0.5:
            ...
            for i, line in enumerate(self._lesson_lines):
                ...
                if la < 0: la = 0
                draw.text((self.MG + 20, 730 + i * lh), line,
                          font=self.f_body, fill=(*self.IVORY, la))
        ...
        if lt > 2.0:
            ...
            draw.text(((self.W - cta_w) // 2, 1010), self.cta,
                      font=self.f_body, fill=(*self.GOLD, ca))
    ...
    def _render(self, t):
        ...
        if t < self.PH1_END:
            ...
            lt3 = t - self.PH2_END
            if lt3 < 1.5:
                br = 0.35 * (1 - self._eo(lt3 / 0.5) * 0.3)  # 반전 시 어둡게
            else:
                br = 0.25 + self._eo((lt3 - 1.5) / 1.0) * 0.1
        else:
            br = 0.30
...
class HistoryCountdownGenerator:
    ...
    def __init__(self, title: str,
        ...
        self._embers = [
            {"x": random.randint(0, self.W), "y": random.randint(self.H // 2, self.H),
             "vx": random.uniform(-6, 6), "vy": random.uniform(-20, -5),
             "r": random.uniform(1, 2.5), "a": random.randint(20, 70),
             "ph": random.uniform(0, math.tau)}
            for _ in range(12)
        ]
    ...
    def _render(self, t):
        ...
        for p in self._embers:
            ...
            fl = 0.3 + 0.7 * math.sin(t * 1.2 + p["ph"])
            al = int(p["a"] * fl)
            draw.ellipse([(x - p["r"], y_ - p["r"]), (x + p["r"], y_ + p["r"])],
                         fill=(212, 165, 116, al))
        ...
        if t < hook_end:
            ...
            for i, line in enumerate(self._title_lines):
                ...
                if la < 0: la = 0
                tw_ = self._tw(line, self.f_title)
                draw.text(((self.W - tw_) // 2, 700 + i * lh), line,
                          font=self.f_title, fill=(*self.IVORY, la))
        elif t < cta_start:
            # Item cards
            lt = t - hook_end
            ...
            draw.text(((self.W - rw) // 2, 450), rn,
                      font=self.f_rank, fill=(*self.GOLD, al))

            # Name
            if item_lt > 0.3:
                na = int(240 * self._eo((item_lt - 0.3) / 0.5))
                nw = self._tw(item["name"], self.f_name)
                draw.text(((self.W - nw) // 2, 580), item["name"],
                          font=self.f_name, fill=(*self.IVORY, na))
            ...
            if item_lt > 0.8 and item["desc_lines"]:
                ...
                for i in range(3):
                    draw.rounded_rectangle(
                        ...
                        radius=8, fill=(*self.PARCHMENT, ca - i * 5))
                draw.rounded_rectangle(
                    [(cx, cy), (cx + self.W - 160, cy + ch)],
                    radius=8, outline=(*self.GOLD, ca // 3), width=1)
                for j, ln in enumerate(item["desc_lines"]):
                    la = int(220 * self._eo((item_lt - 1.0 - j * 0.1) / 0.4))
                    if la < 0: la = 0
                    draw.text((cx + 25, cy + 20 + j * lh), ln,
                              font=self.f_body, fill=(*self.IVORY, la))
        else:
            # CTA
            clt = t - cta_start
            ...
            draw.text(((self.W - cw) // 2, 800), self.cta,
                      font=self.f_body, fill=(*self.GOLD, ca))
...
def main():
    ...
    if a.demo in ("fact", "all"):
        ...
        HistoryFactGenerator(**DEMO_FACT).generate(
            a.out if a.demo == "fact" else "output/demo_history_fact.mp4"
        )
