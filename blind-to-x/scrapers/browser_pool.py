...
class _PooledPage:
    ...
    def __init__(self, page: Any, pool: "BrowserContextPool", ctx: Any = None):
        self._page = page
        self._pool = pool
...
class BrowserContextPool:
    ...
    async def open(self) -> None:
        ...
        self._ready = True
        logger.info("BrowserContextPool opened: %d context slots (requested %d, stealth applied)", len(self._contexts), self.size)
    ...
    def _release(self, ctx: Any = None) -> None:
        """Return a specific context slot to the pool (called by _PooledPage.close()).

        Args:
            ctx: The specific browser context to return. If None, returns contexts[0]
                 as a fallback (should not happen in normal operation).
        """
        target = ctx if ctx is not None else (self._contexts[0] if self._contexts else None)
        if target is not None:
            try:
                self._queue.put_nowait(target)
            except asyncio.QueueFull:
                logger.debug("BrowserContextPool._release: queue full (unexpected), context dropped")
