...
async def test_run_dry_run_single_success(mock_deps, monkeypatch):
    ...
    monkeypatch.setattr(pipeline.commands.dry_run, "build_review_decision", lambda c, pd, p: {"should_queue": True, "review_reason": "", "review_status": "approved"})
...
async def test_run_dry_run_single_filter_low_quality(mock_deps, monkeypatch):
    ...
    import pipeline.commands.dry_run
    monkeypatch.setattr(pipeline.commands.dry_run, "build_review_decision", lambda c, pd, p: {"should_queue": False, "review_reason": "low_score", "review_status": "rejected"})
