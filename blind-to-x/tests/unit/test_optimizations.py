...
class TestNotionURLCache:
    ...
    async def test_cache_built_once_on_first_call(self):
        ...
        fake_page = {
            "properties": {
                "Source URL": {"type": "url", "url": "https://www.teamblind.com/post/test-1"}
            }
        }
...
class TestClassificationRulesYAML:
    ...
    def test_topic_rules_fallback_when_no_yaml(self, tmp_path, monkeypatch):
        """YAML 파일 없을 때 내장 fallback 규칙이 반환되는지 확인."""
        import pipeline.content_intelligence as ci

        # 캐시 초기화
        ci._loaded_rules = None
        # 존재하지 않는 경로로 override
        monkeypatch.setattr(ci, "_RULES_FILE", tmp_path / "nonexistent.yaml")
    ...
    def test_topic_rules_loaded_from_yaml(self, tmp_path, monkeypatch):
        """YAML 파일이 있을 때 해당 규칙이 로드되는지 확인."""
        import pipeline.content_intelligence as ci

        ci._loaded_rules = None
        rules_file = tmp_path / "classification_rules.yaml"
        ...
        monkeypatch.setattr(ci, "_RULES_FILE", rules_file)
    ...
    def test_classify_topic_cluster_uses_yaml_rules(self, tmp_path, monkeypatch):
        """classify_topic_cluster()가 YAML 규칙을 실제로 사용하는지 확인."""
        import pipeline.content_intelligence as ci
...
class TestDraftGeneratorCache:
    ...
    async def test_second_call_uses_cache(self):
        ...
        async def mock_gemini(_prompt):
            nonlocal call_count
            call_count += 1
            return "<twitter>테스트 트윗</twitter>", 100, 50
    ...
    async def test_different_content_not_cached(self):
        ...
        async def mock_gemini(_prompt):
            nonlocal call_count
            call_count += 1
            return "<twitter>트윗 내용</twitter>", 100, 50
