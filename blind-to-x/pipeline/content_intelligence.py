...
def _load_rules() -> dict:
    """classification_rules.yaml을 1회 로드 후 캐시. 파일 없으면 빈 dict 반환."""
    ...
    try:
        ...
        logger.warning("classification_rules.yaml 없음. 코드 내장 규칙 사용.")
        _loaded_rules = {}
    except Exception as exc:
        logger.warning("classification_rules.yaml 로드 실패: %s. 코드 내장 규칙 사용.", exc)
        _loaded_rules = {}
    return _loaded_rules
...
def calculate_publishability_score(post_data: dict[str, Any], topic_cluster: str, hook_type: str, emotion_axis: str) -> tuple[float, list[str]]:
    ...
    score = 30.0  # 기본 점수 (25→30): 품질 게이트 완화 조정

    if 10 <= len(title.strip()) <= 40:
        score += 12
        rationale.append("title_length_fit")
    elif 8 <= len(title.strip()) <= 45:
        score += 6
        rationale.append("title_length_marginal")

    if topic_cluster != "기타":
        score += 15  # 토픽 감지 가중 상향 (8→15)
        rationale.append("topic_cluster_detected")

    if hook_type in {"공감형", "논쟁형"}:
        score += 12
        rationale.append("strong_hook")
    elif hook_type == "정보형":
        score += 9
        rationale.append("actionable_hook")
    else:
        score += 3  # 약한 훅 감점 (5→3)

    if emotion_axis in {"공감", "분노", "현타", "웃김"}:
        score += 10
        rationale.append("emotionally_resonant")

    if re.search(r"(회사|직장|상사|퇴사|이직|연봉|면접|팀장|출근|재택)", f"{title} {content}"):
        score += 18  # 직장인 관련성 상향 (12→18)
        rationale.append("workplace_relevance")

    engagement = _engagement_signal(post_data)
    score += engagement
    if engagement > 0:
        rationale.append("engagement_signal")

    if len(content.strip()) >= 150:
        score += 7
        rationale.append("enough_context")
    elif len(content.strip()) >= 120:
        score += 3
        rationale.append("minimal_context")

    return _round_score(score), rationale
...
def build_content_profile(
    ...
    publishability_score, publishability_rationale = calculate_publishability_score(
        post_data,
        topic_cluster,
        hook_type,
        emotion_axis,
    ...
    if llm_viral_boost and 50 <= publishability_score <= 70:
        ...
        if boost > 0:
            ...
            publishability_rationale.append("llm_viral_boost")
