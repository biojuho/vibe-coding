...
class FeedbackLoop:
    ...
    async def compute_adaptive_weights(self) -> dict[str, float]:
        ...
        records = await self.notion_uploader.get_top_performing_posts(
            limit=50, lookback_days=int(self.config.get("analytics.lookback_days", 30)), minimum_posts=0,
        )
        # 실제 성과 데이터가 있는 레코드만 사용
        valid = [
            r for r in records
            if r.get("views") and float(r.get("views") or 0) > 0
            and r.get("scrape_quality_score") and r.get("publishability_score") and r.get("performance_score")
        ]
        if len(valid) < 5:
    ...
    async def get_few_shot_examples(self, limit: int | None = None) -> list[dict[str, Any]]:
        ...
        return await self.notion_uploader.get_top_performing_posts(
            limit=top_limit,
    ...
    def analyze_success_patterns(
        ...
        winners = [
            r for r in records
            if r.get("performance_grade", "D") in grade_filter
        ]
        if not winners:
        ...
        style_c = Counter(
            r.get("chosen_draft_type") or r.get("recommended_draft_type") or "공감형"
            for r in winners
        )
