...
class TrendMonitor:
    ...
    def __init__(self, config: Any = None):
        ...
        self._topic_keywords: dict[str, list[str]] = self._get(
            "trends.topic_keywords", _DEFAULT_TOPIC_KEYWORDS
        )
            ...
            for attempt in range(_MAX_RETRIES):
                try:
                    ...
                    if attempt < _MAX_RETRIES - 1:
                        backoff = 2 ** (attempt + 1)
                        logger.debug("Google Trends trending_searches 재시도 %d/%d (%.0fs): %s", attempt + 1, _MAX_RETRIES, backoff, exc)
                        _time.sleep(backoff)
                    else:
            ...
            for i in range(0, len(all_keywords), 5):
                ...
                for attempt in range(_MAX_RETRIES):
                    try:
                        ...
                        if attempt < _MAX_RETRIES - 1:
                            backoff = 2 ** (attempt + 1)
                            logger.debug("Google Trends interest_over_time 재시도 %d/%d (%s): %s", attempt + 1, _MAX_RETRIES, batch, exc)
                            _time.sleep(backoff)
                        else:
            ...
            try:
                ...
                async with aiohttp.ClientSession(timeout=timeout) as session:
                    async with session.post(
                        _NAVER_DATALAB_URL, json=payload, headers=headers
                    ) as resp:
                        if resp.status != 200:
    ...
    async def get_trending_keywords(self) -> dict[str, float]:
        ...
        google_result, naver_result = await asyncio.gather(
            google_task, naver_task, return_exceptions=True
        )
