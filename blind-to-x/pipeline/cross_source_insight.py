...
5. 반드시 <twitter> 와 </twitter> 태그 안에만 작성하세요.
"""
...
"""
    ...
    if "naver_blog" in output_formats:
        naver_blog_block = """
[네이버 블로그 트렌드 분석 초안]
1. 1500~3000자, SEO 최적화 구조 (소제목 3개 이상)로 작성하세요.
2. 각 커뮤니티의 관점을 비교 분석하는 형식으로 작성하세요.
3. 데이터/통계 느낌의 표현을 사용하세요 ("N개 커뮤니티에서 동시에...").
4. SEO 태그 10개 이상 포함하세요.
5. 반드시 <naver_blog> 와 </naver_blog> 태그 안에만 작성하세요.
"""

    prompt = f"""당신은 직장인 커뮤니티 트렌드 분석 전문가입니다.
        ...
        for attempt in range(1, draft_generator.max_retries_per_provider + 1):
            try:
                logger.info(
                    ...
                    provider, attempt, draft_generator.max_retries_per_provider,
                    trend_group["topic_cluster"],
                )
                response_text, input_tokens, output_tokens = await draft_generator._generate_once(
                    provider, prompt
                )
                ...
                drafts, image_prompt = draft_generator._parse_response(
                    response_text, output_formats, provider
                )
                # 태그 파싱 검증: 주요 플랫폼 키가 비어있으면 경고
                _empty_platforms = [
                    fmt for fmt in output_formats
                    if fmt in drafts and (not drafts[fmt] or not drafts[fmt].strip())
                ]
                if _empty_platforms:
                    logger.warning(
                        "인사이트 파싱 불완전 — 빈 플랫폼: %s (provider=%s, topic=%s)",
                        ", ".join(_empty_platforms), provider, trend_group["topic_cluster"],
                    )
                ...
                logger.warning(
                    ...
                    provider, attempt, draft_generator.max_retries_per_provider, exc,
                )
                import asyncio
...
async def process_cross_source_insights(
    ...
    for group in trend_groups:
        ...
        drafts, image_prompt = await generate_insight_draft(
            group, draft_generator, output_formats, top_examples
        )
