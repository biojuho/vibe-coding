...
function fail(name, error) {
	...
	if (error?.cause) {
		const causeMessage =
			error.cause?.message || error.cause?.toString?.() || String(error.cause);
...
async function testClientGeneration() {
	...
	try {
		...
		assert.equal(
			missing.length,
			0,
			`Missing model delegates: ${missing.join(", ")}`,
		);
...
async function testAdapterConstruction() {
	...
	try {
		const adapter = new PrismaPg({
			connectionString:
				"postgresql://user:pass@pooler.supabase.com:6543/postgres",
			ssl: { rejectUnauthorized: false },
			pool: {
				max: 10,
				idleTimeout: 20,
...
async function testGracefulErrors() {
	...
	try {
		const adapter = new PrismaPg({
			connectionString:
				"postgresql://invalid:invalid@127.0.0.1:59999/nonexistent",
	...
	try {
		...
		try {
			...
			assert.ok(
				...
				"Buildings query should return array",
			);
		...
		try {
			...
			// Cleanup
			await prisma.dashboardSnapshot
				.delete({ where: { key: txTestKey } })
				.catch(() => {});
			await prisma.notificationSummary
				.delete({ where: { key: txTestKey } })
				.catch(() => {});
		} catch (e) {
			fail("Transaction", e);
		...
		try {
			const queries = Array.from({ length: 10 }, () => prisma.building.count());
