...
function evaluateExpectations(indexRows, expectations) {
	return expectations.map((expectation) => {
		const matched = indexRows.find((row) =>
			matchesExpectation(row.indexdef, expectation.fragments),
		);
		return {
			...expectation,
...
async function main() {
	...
	if (isPlaceholderDatabaseUrl(databaseUrl)) {
		console.error(
			"DATABASE_URL is missing or still uses the placeholder value in projects/hanwoo-dashboard/.env.",
		);
		console.error(
			"Add the real pooled Postgres URL first, then rerun `npm run db:verify-indexes`.",
		);
		process.exitCode = 2;
		return;
	}
	...
	try {
		...
		const schemaResults = evaluateExpectations(
			indexRows,
			schemaIndexExpectations,
		);
