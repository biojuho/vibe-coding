...
async function main() {
		...
		for (let pen = 1; pen <= 12; pen++) {
			...
			for (let c = 0; c < cnt; c++) {
				...
				if (st === "번식우" && isFemale) {
					...
					lastEstrus.setDate(
						lastEstrus.getDate() - Math.floor(Math.random() * 21),
					);
				}
				...
				if (st === "임신우") {
					...
					pregnancyDate.setMonth(
						pregnancyDate.getMonth() - Math.floor(Math.random() * 8) - 1,
					);
				}
				...
				purchaseDate.setMonth(
					purchaseDate.getMonth() + 1 + Math.floor(Math.random() * 2),
				);
	...
	const sellableCattle = createdCattle.filter(
		(c) => c.status === "비육우" || c.status === "씨수소",
	);
	...
	for (let i = 0; i < saleCount; i++) {
		...
		const salePrice = Math.floor(
			((basePrice * saleWeight) / 1000) * (0.9 + Math.random() * 0.2),
		);
		...
		await prisma.salesRecord.create({
			data: {
				...
				purchaser:
					["남원", "장수", "익산"][Math.floor(Math.random() * 3)] +
					" 축산물공판장",
				grade,
				cattleId: cow.id,
			},
		...
		for (const cat of categories) {
			...
			for (let i = 0; i < count; i++) {
				...
				if (cat === "feed") {
					...
					cattleId =
						createdCattle[Math.floor(Math.random() * createdCattle.length)].id;
				}
