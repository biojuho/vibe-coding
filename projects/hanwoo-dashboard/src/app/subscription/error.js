...
export default function SubscriptionError(options = {}) {
	const { error, reset } = normalizeErrorOptions(options);
	const safeReset = normalizeReset(reset);
	const retryLabel = "결제 페이지 다시 불러오기";

	useEffect(() => {
		console.error("[hanwoo-dashboard] subscription error:", error);
	}, [error]);
			...
			>
				<div className="login-brand">
					...
					<div>
						...
						<h1 id="subscription-error-title" className="login-title">
							결제 정보를 불러오지 못했습니다
						</h1>
					</div>
				</div>
				...
				<p className="login-copy">
					구독 정보를 가져오는 중 오류가 발생했습니다. 잠시 후 다시 시도해
					주세요. 결제는 처리되지 않은 상태입니다.
				</p>
