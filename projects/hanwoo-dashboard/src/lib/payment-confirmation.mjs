...
// Toss returns this 4xx code when a payment was already approved — typically a
// retry after our first confirm response was lost (timeout/5xx). It must NOT be
// treated as a terminal failure; the money was charged, so the route reconciles
// it against Toss and activates the subscription instead of marking it FAILED.
export const TOSS_ALREADY_PROCESSED_CODE = "ALREADY_PROCESSED_PAYMENT";
...
export function classifyPaymentConfirmationResult(input = {}) {
	...
	if (status >= 400) {
		if (
			...
			payload.code === TOSS_ALREADY_PROCESSED_CODE
		) {
			// Already charged at Toss — reconcile instead of failing.
			return {
