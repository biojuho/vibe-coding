...
test('analysis tab normalizes numeric inputs before financial and feed aggregation', () => {
  ...
  assert.match(analysisSource, /return Array\.isArray\(items\) \? items\.filter\(\(item\) => item && typeof item === 'object'\) : \[\];/);
  assert.match(analysisSource, /const safeSaleRecords = useMemo\(\(\) => normalizeAnalysisItems\(saleRecords\), \[saleRecords\]\);/);
  assert.match(analysisSource, /const safeFeedHistory = useMemo\(\(\) => normalizeAnalysisItems\(feedHistory\), \[feedHistory\]\);/);
  assert.match(analysisSource, /const safeCattleList = useMemo\(\(\) => normalizeAnalysisItems\(cattleList\), \[cattleList\]\);/);
  assert.match(analysisSource, /const safeExpenseRecords = useMemo\(\(\) => normalizeAnalysisItems\(expenseRecords\), \[expenseRecords\]\);/);
  assert.match(analysisSource, /const hasExpenseData = safeExpenseRecords\.length > 0;/);
  ...
  assert.match(analysisSource, /safeExpenseRecords\.forEach\(\(expense\) => \{/);
...
test('financial chart widget normalizes numeric inputs before chart aggregation', () => {
  ...
  assert.match(source, /return Array\.isArray\(items\)\s+\? items\.filter\(\(item\) => item && typeof item === 'object'\)\s+: \[\];/);
  assert.match(source, /const safeSaleRecords = normalizeFinancialChartItems\(saleRecords\);/);
  assert.match(source, /const safeCostRecords = normalizeFinancialChartItems\(expenseRecords\);/);
  assert.match(source, /const safeSeriesData = normalizeFinancialChartItems\(seriesData\);/);
