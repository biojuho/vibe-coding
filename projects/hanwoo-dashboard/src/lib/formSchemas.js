...
const toDefaultInputDate = (date = new Date()) =>
	toInputDate(date) || toInputDate(new Date());
...
const optionalText = (max = 300) =>
	z.preprocess(
		emptyToUndefined,
		z.string().max(max, `${max}자 이하로 입력해 주세요.`).optional(),
	);
...
export const inventoryItemSchema = z.object({
	...
	quantity: z.preprocess(
		toPlainNumber,
		z.number().positive("수량은 0보다 커야 합니다."),
	),
...
export const feedRecordSchema = z.object({
	...
	roughage: z.preprocess(
		toPlainNumber,
		z.number().positive("조사료 양은 0보다 커야 합니다."),
	),
	concentrate: z.preprocess(
		toPlainNumber,
		z.number().positive("배합사료 양은 0보다 커야 합니다."),
	),
...
export const buildingFormSchema = z.object({
	...
	penCount: z.preprocess(
		...
		z
			.number()
			.int()
			.min(1, "칸 수는 1 이상이어야 합니다.")
			.max(200, "칸 수를 확인해 주세요."),
	),
});
