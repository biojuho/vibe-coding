...
export async function fetchMarketPrice() {
	...
	if (!apiKey) {
		console.warn(
			"KAPE_SERVICE_KEY not found in .env. Skipping live market price lookup.",
		);
		return null;
	}
	...
	try {
		...
		for (let dayOffset = 1; dayOffset <= 7; dayOffset++) {
			...
			if (elapsedMs >= KAPE_TOTAL_DEADLINE_MS) {
				throw new Error(
					`KAPE lookup exceeded ${KAPE_TOTAL_DEADLINE_MS}ms overall deadline.`,
				);
			}
			...
			try {
				...
				console.warn(
					`KAPE API returned unreadable JSON for date ${issueDate}:`,
					error,
				);
				continue;
			}
