...
const boundedNumber = (
	message,
	{ integer = false, positive = false, nonnegative = false, min, max } = {},
) => {
...
const requiredPositiveInt = (message, max) =>
	z.preprocess(
		toNumber,
		boundedNumber(message, { integer: true, positive: true, max }),
	);

const optionalNonNegativeNumber = (message, max) =>
	z.preprocess(
		toNumber,
		boundedNumber(message, { nonnegative: true, max }).optional(),
	);

const optionalNonNegativeInt = (message, max) =>
	z.preprocess(
		toNumber,
		boundedNumber(message, {
			integer: true,
			nonnegative: true,
			max,
		}).optional(),
...
function validateActionInput(
	schema,
	input,
	fallbackMessage = "입력값을 다시 확인해 주세요.",
) {
...
const farmSettingsSchema = z.object({
	...
	latitude: z.preprocess(
		toNumber,
		boundedNumber("위도를 확인해 주세요.", { min: -90, max: 90 }),
	),
	longitude: z.preprocess(
		toNumber,
		boundedNumber("경도를 확인해 주세요.", { min: -180, max: 180 }),
	),
});
