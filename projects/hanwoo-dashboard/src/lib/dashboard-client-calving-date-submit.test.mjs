...
test('dashboard calving submit avoids raw invalid Date toISOString calls', () => {
  ...
  assert.match(source, /const birthDate = toStrictIsoDateOrNull\(calvingDate\);/);
  assert.match(source, /if \(!birthDate\) \{\s+showError\('[^']*'\);\s+return false;\s+\}/);
  assert.match(source, /birthDate,/);
  assert.doesNotMatch(source, /birthDate: new Date\(calvingDate\)\.toISOString\(\)/);
});
