...
function resolveLocalModule(specifier, fromFile) {
	...
	if (specifierExt && !RESOLVE_EXTS.includes(specifierExt)) {
		if (IGNORED_SOURCE_EXTS.has(specifierExt))
			return { absolute, ignored: true };
	}
...
function sliceTopImportBlock(source) {
	...
	for (const rawLine of lines) {
		...
		if (!seenCode) {
			...
			if (/^['"]use (client|server|strict)['"];?$/.test(trimmed)) {
				keep.push(line);
				continue;
			}
			...
			if (/^(import|export)\b.*\bfrom\s+['"][^'"]+['"]/.test(trimmed)) {
				keep.push(line);
				continue;
			}
...
const FROM_RE =
	/(import|export)\s+(?:type\s+)?([\s\S]*?)\bfrom\s+['"]([^'"]+)['"]/g;
...
test("component files are present for import smoke check", () => {
	assert.ok(
		files.length > 0,
		"expected at least one component file under src/components or src/app",
	);
});
...
test("every local import resolves to an existing file", () => {
	...
	for (const file of files) {
		...
		for (const stmt of statements) {
			...
			if (!resolved) {
				problems.push(
					`${path.relative(PROJECT_ROOT, file)} → '${spec}' (unresolved)`,
				);
			}
		}
	}
