...
test("dashboard stacking context keeps mobile tab bar fixed", () => {
	...
	assert.match(
		source,
		/\.settings-widget-card\s*\{[\s\S]*?--settings-widget-card-padding:\s*18px 20px;[\s\S]*?--settings-widget-card-margin-bottom:\s*20px;[\s\S]*?--settings-widget-title-margin-bottom:\s*14px;[\s\S]*?--settings-widget-description-margin-bottom:\s*12px;/,
	);
	assert.match(
		source,
		/\.settings-farm-form\s*\{[\s\S]*?--settings-farm-form-margin-top:\s*0;[\s\S]*?--settings-farm-form-margin-bottom:\s*30px;/,
	);
	assert.match(
		source,
		/@media \(max-width:\s*640px\)\s*\{[\s\S]*?\.settings-widget-card\s*\{[\s\S]*?--settings-widget-card-padding:\s*12px 16px;[\s\S]*?--settings-widget-card-margin-bottom:\s*12px;[\s\S]*?--settings-widget-title-margin-bottom:\s*8px;[\s\S]*?--settings-widget-description-margin-bottom:\s*8px;[\s\S]*?\.settings-widget-grid-viewport\s*\{[\s\S]*?max-height:\s*72px;[\s\S]*?\.settings-farm-form\s*\{[\s\S]*?--settings-farm-form-margin-bottom:\s*112px;[\s\S]*?\.settings-farm-fields-viewport\s*\{[\s\S]*?max-height:\s*128px;[\s\S]*?overflow-y:\s*auto;[\s\S]*?scroll-padding-bottom:\s*64px;/,
	);
	assert.match(
		source,
