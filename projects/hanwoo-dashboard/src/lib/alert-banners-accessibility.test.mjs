...
test('alert banners normalize malformed notification and building payloads before rendering', () => {
  ...
  assert.match(source, /function normalizeAlertNotifications\(notifications, type\)/);
  assert.match(source, /if \(!Array\.isArray\(notifications\)\) return \[\]/);
  assert.match(source, /notification && typeof notification === 'object' && notification\.type === type/);
  assert.match(source, /function normalizeBuildings\(buildings\)/);
  assert.match(source, /Array\.isArray\(buildings\)/);
  assert.match(source, /const safeBuildings = normalizeBuildings\(buildings\);/);
  assert.match(source, /normalizeAlertNotifications\(notifications, 'estrus'\)/);
  assert.match(source, /normalizeAlertNotifications\(notifications, 'calving'\)/);
  assert.match(source, /safeBuildings\.find\(\(item\) => item\.id === notification\.buildingId\)/);
  assert.match(source, /id: notification\.id \?\? `\$\{type\}-\$\{index\}`/);
  assert.match(source, /'이름 미등록'/);
  assert.doesNotMatch(source, /notifications\.filter\(\(notification\) => notification\.type/);
  assert.doesNotMatch(source, /buildings\.find\(\(item\) => item\.id === notification\.buildingId\)/);
});
