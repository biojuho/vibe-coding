...
import { X, Camera, RefreshCw, AlertTriangle, Eye, Check } from 'lucide-react';
import { playScanSuccess, playScanFailure, playTactileClick, triggerVibration } from '@/lib/audio';
import { PremiumButton } from '@/components/ui/premium-button';

export default function EarTagScannerModal({ isOpen, onClose, cattleList = [], onSelect }) {
  ...
  useEffect(() => {
    ...
    // OCR digits rolling
    const ocrCodes = ['0025184', '3910582', '8273910', '0029314', '7491028', '1947201'];
    ...
    const render = () => {
      ...
      ctx.fillStyle = frameCount % 30 < 15 ? 'rgba(34, 197, 94, 0.3)' : 'rgba(34, 197, 94, 0.1)';
      ctx.beginPath();
      ...
      ctx.fillText(`MOCK_RAW: ${ocrCodes[ocrIndex]}${Math.floor(Math.random() * 10)}`, width - pad - 130, height - pad + 20);
  ...
  return (
    ...
    >
      <div 
        ...
        style={{ background: 'linear-gradient(135deg, var(--color-card), #162416)' }}
      >
        {/* Header */}
        <div className="flex justify-between items-center px-6 py-5 border-b border-emerald-500/10">
          <div className="flex items-center gap-2.5">
            ...
            <h2 id="scanner-modal-title" className="text-base font-extrabold text-foreground tracking-tight">
              가상 이표 인식 스캐너
            </h2>
          </div>
          <button
        ...
        <div className="p-6 flex flex-col items-center justify-center flex-1">
          {scanStatus === 'scanning' ? (
            ...
            <div className="w-full text-center py-4 flex flex-col items-center animate-fadeIn">
              ...
              <h3 className="text-lg font-black text-emerald-400 mb-1.5">이표 인식 완료!</h3>
              <p className="text-[12px] text-muted-foreground mb-5">농림축산검역본부 표준 개체 감지</p>
              ...
              <div className="w-full p-5 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 text-left mb-6 relative overflow-hidden shadow-md">
                ...
                <h4 className="text-xl font-bold text-foreground mb-3">{matchedCow?.name}</h4>
                <div className="grid grid-cols-2 gap-3 text-xs border-t border-emerald-500/20 pt-3">
                  <div>
                    ...
                    <span className="font-semibold text-foreground">{matchedCow?.status}</span>
                  </div>
                  <div>
                    <span className="text-muted-foreground block">위치</span>
                    <span className="font-semibold text-foreground">
                      {matchedCow?.buildingId ? `${matchedCow?.buildingId}동` : '미배정'} {matchedCow?.penNumber ? `${matchedCow?.penNumber}번 칸` : ''}
                    </span>
                  </div>
                  <div>
                    <span className="text-muted-foreground block">현재 체중</span>
                    <span className="font-semibold text-foreground">{matchedCow?.weight}kg</span>
                  </div>
                  <div>
                    <span className="text-muted-foreground block">생년월일</span>
                    <span className="font-semibold text-foreground">
                      {matchedCow?.birthDate ? new Date(matchedCow.birthDate).toLocaleDateString('ko-KR') : '-'}
                    </span>
                  </div>
                </div>
              </div>
            ...
            <div className="w-full text-center py-6 flex flex-col items-center animate-fadeIn">
              ...
              <h3 className="text-base font-bold text-foreground mb-2">인식된 개체 정보 없음</h3>
              <p className="text-xs text-muted-foreground max-w-xs mb-6 leading-relaxed">
                스캐너 프레임 안에 이표번호가 정확히 위치하지 않았거나 농장에 등록되지 않은 번호입니다. 다시 스캔해주십시오.
              </p>
            </div>
          )}
        </div>
        ...
        {scanStatus === 'scanning' && cattleList.length > 0 && (
          <div className="px-6 pb-2 text-center">
            <span className="text-[10px] text-muted-foreground block mb-2">시뮬레이션 전용: 강제 타겟 설정</span>
