...
import { Search, Camera, CheckCircle2, ClipboardList, Thermometer, ShieldAlert, Sparkles, Clock, AlertTriangle, ArrowLeft } from 'lucide-react';
import { playTactileClick, triggerVibration, playTriumphantChime } from '@/lib/audio';
import EarTagScannerModal from './EarTagScannerModal';

const DEFAULT_CHECKLIST = [
  { id: 'feed_check', title: '우사 급이통 청소 및 사료 배부 확인', icon: '🌾', detail: '오전 급여 후 남은 찌꺼기 및 사료 변질 여부' },
  { id: 'bedding_check', title: '우방 깔짚(톱밥) 수분 상태 점검', icon: '🧹', detail: '축사 바닥 톱밥 오염도 측정 및 추가 보충 여부' },
  { id: 'thi_monitor', title: '온습도 및 가축 열지수(THI) 모니터링', icon: '🌬️', detail: 'THI 경보 확인 후 환풍기 풍량 및 안개분무 작동' },
  { id: 'health_scan', title: '개체 거동 및 활력 관찰 (건강 점검)', icon: '🩺', detail: '절뚝임, 호흡 급박, 사료 섭취 거부 개체 식별' },
  { id: 'breeding_focus', title: '임신/분만 예정우 상태 밀착 관찰', icon: '🍼', detail: '유방 팽창, 외음부 부종, 진통 개시 징후 점검' },
];
...
export default function FieldModeView({ 
  ...
  const [loadingAllCattle, setLoadingAllCattle] = useState(!!ensureAllCattleLoaded);
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      ...
      if (saved) {
        ...
        const fresh = DEFAULT_CHECKLIST.map(item => ({ ...item, checked: false }));
        setChecklist(fresh);
  ...
  useEffect(() => {
    ...
    const t1 = setTimeout(() => createFirework(width * 0.25, height * 0.6), 250);
    const t2 = setTimeout(() => createFirework(width * 0.75, height * 0.6), 400);
  ...
  const handleToggleCheck = (itemId) => {
    ...
    const previouslyCompletedAll = checklist.length > 0 && checklist.every(item => item.checked);
    const currentlyCompletedAll = updated.length > 0 && updated.every(item => item.checked);
  ...
  const statsSummary = useMemo(() => {
    const estrusCount = cattleList.filter(cow => {
      ...
      const days = Math.floor((new Date() - new Date(cow.lastEstrus)) / (1000 * 60 * 60 * 24));
      return days >= 19 && days <= 23; // standard estrus cycle alarm range
    }).length;

    const calvingCount = cattleList.filter(cow => cow.status === '임신우').length;
  ...
  const formatTagNumber = (tag = '') => {
    ...
    return (
      <span className="font-mono tracking-tight text-xs opacity-75">
        {prefix}
        <strong className="text-[var(--color-primary-custom)] font-extrabold text-sm ml-0.5" style={{ textShadow: '0 0 8px rgba(245, 158, 11, 0.4)' }}>
          {suffix}
        </strong>
      </span>
    );
  ...
  return (
    ...
    >
      ...
      >
        <button
          type="button"
          onClick={() => { playTactileClick(); onCloseFieldMode(); }}
        ...
        <div className="text-center">
          <div className="text-[9px] text-amber-500/80 font-black tracking-widest uppercase">Smart Field Overlay</div>
          <h2 className="text-sm font-black text-amber-300 tracking-tight flex items-center gap-1.5 justify-center">
            현장 스마트 모드 <span className="inline-block w-2 h-2 rounded-full bg-amber-400 animate-ping" />
          </h2>
        </div>
      ...
      <div className="max-w-[540px] mx-auto px-5 pt-6 flex flex-col gap-6">
        ...
        <div 
          ...
          style={{ background: 'linear-gradient(135deg, rgba(28, 43, 28, 0.7), rgba(10, 14, 10, 0.8))' }}
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-xs text-amber-400/80 font-bold">농장 온습도 지수</div>
          ...
          <div className="text-right">
            <span className="text-[10px] text-amber-500 block font-bold">오프라인 자가생존</span>
        ...
        <section aria-labelledby="global-search-title" className="flex flex-col gap-3">
          <h3 id="global-search-title" className="sr-only">개체 초고속 검색</h3>
          <div className="flex gap-2">
            <div className="relative flex-1">
              ...
              />
              <Search className="absolute left-4 top-4.5 text-amber-500" size={18} />
              
              {searchQuery && (
                <button
                  type="button"
                  onClick={() => { playTactileClick(); setSearchQuery(''); }}
            ...
            <button
              type="button"
              onClick={() => { playTactileClick(); setIsScannerOpen(true); }}
          ...
          {searchQuery && (
            ...
            >
              <div className="px-4.5 py-3 border-b border-amber-500/10 bg-amber-500/5 flex justify-between items-center">
                <span className="text-[11px] text-amber-400 font-black">검색 매칭 개체 ({filteredCattle.length}두)</span>
                {loadingAllCattle && <span className="text-[10px] text-amber-400 animate-pulse">전체 로드 중...</span>}
              </div>
                ...
                {filteredCattle.length > 0 ? (
                  filteredCattle.map(cow => (
                    <button
                      key={cow.id}
                      type="button"
                      onClick={() => { playTactileClick(); onSelect(cow); }}
                    ...
                    >
                      <div>
                        <div className="flex items-center gap-2 mb-1.5">
                          <span className="font-extrabold text-sm text-amber-200">{cow.name}</span>
                          <span 
                        ...
                        <div className="flex items-center gap-1.5 text-xs text-amber-400/70">
                          ...
                          <span>·</span>
                          <span>
                            {cow.buildingId ? `${cow.buildingId}동` : '미지정'} {cow.penNumber ? `${cow.penNumber}번 칸` : ''}
                          </span>
                        </div>
                      </div>
                      <span className="text-amber-500 text-lg opacity-60 group-hover:opacity-100 group-hover:translate-x-1 transition-all">›</span>
                    </button>
                  ))
                ) : (
                  <div className="px-5 py-8 text-center text-xs text-amber-500/50">
                    <AlertTriangle size={20} className="mx-auto mb-2 text-amber-500/40" />
                    매칭되는 개체가 존재하지 않습니다.
                  </div>
                )}
              </div>
        ...
        <section aria-labelledby="checklist-title" className="flex flex-col gap-3">
          <div 
            ...
            style={{ background: 'linear-gradient(135deg, rgba(28, 43, 28, 0.6), rgba(10, 14, 10, 0.85))' }}
          >
            <div className="flex justify-between items-start">
              <div>
                <div className="text-[10px] text-amber-500 font-bold uppercase tracking-wide">Tactile stables list</div>
                <h3 id="checklist-title" className="text-base font-extrabold text-amber-300 mt-0.5">
                  오늘의 축사 일일 점검
                </h3>
              </div>
              <div className="text-right">
                <span className="text-xl font-black text-amber-300">{checklistStats.pct}%</span>
                <span className="text-[10px] text-amber-500 block">완료 ({checklistStats.checked}/{checklistStats.total})</span>
              </div>
            </div>

            {/* Glowing progress track */}
            <div className="w-full h-2.5 bg-amber-950/40 rounded-full border border-amber-500/10 overflow-hidden relative" aria-hidden="true">
              <span 
                ...
                >
                  <div className="flex items-center gap-3.5 flex-1 min-w-0">
                    ...
                    <div className="min-w-0">
                      <h4 className={`text-[13px] font-bold tracking-tight leading-normal ${item.checked ? 'text-amber-400 line-through opacity-65' : 'text-foreground'}`}>
                        {item.title}
                      </h4>
                  ...
                  <div 
                    ...
                    aria-hidden="true"
                  >
                    {item.checked ? <CheckCircle2 size={14} strokeWidth={3.5} /> : null}
                  </div>
                </button>
              ))}
            </div>
        ...
        <section aria-labelledby="status-counters-title" className="grid grid-cols-2 gap-3.5">
          <h3 id="status-counters-title" className="sr-only">축사 개체 모니터 카운터</h3>
          
          <div 
            className="p-4.5 rounded-[24px] border border-amber-500/20"
            style={{ background: 'linear-gradient(135deg, rgba(28, 43, 28, 0.4), rgba(10, 14, 10, 0.8))' }}
          >
            <div className="text-[10px] text-amber-500/80 font-bold">전체 사육개체</div>
            <div className="text-2xl font-black text-amber-300 mt-1 font-mono tracking-tight">
              {cattleList.length}두
            </div>
            <div className="text-[10px] text-amber-500/60 mt-1">이표 검수 100% 완료</div>
          </div>
          ...
          <div 
            ...
            style={{ background: 'linear-gradient(135deg, rgba(28, 43, 28, 0.4), rgba(10, 14, 10, 0.8))' }}
          >
            <div className="text-[10px] text-amber-500/80 font-bold">임신/분만 대기</div>
            <div className="text-2xl font-black text-amber-300 mt-1 font-mono tracking-tight">
              {statsSummary.calvingCount}두
            </div>
            <div className="text-[10px] text-amber-500/60 mt-1">가축 기상 경보 확인 요망</div>
          </div>
        </section>
      </div>
