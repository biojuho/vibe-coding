...
const PAYMENT_WIDGET_TIMEOUT_MESSAGE =
...
export default function PaymentWidget({
	...
	const handlePayment = async () => {
		...
		try {
			...
			const { data: preparedPayment, rawText } =
				await readJsonResponseSafely(prepareResponse);
			...
			{errorMessage ? (
				<p
					...
					style={{
						...
						padding: "12px 14px",
						background:
							"color-mix(in srgb, var(--color-danger, #dc2626) 14%, white 86%)",
