...
async function run() {
	...
	try {
		...
		assert(
			pageHtml.toLowerCase().includes("<html"),
			"root page should render the app shell",
		);

		const unauthorizedDashboard = await fetch(
			`${BASE_URL}/api/data/dashboard`,
			{ redirect: "manual" },
		);
		assert.equal(
			unauthorizedDashboard.status,
			401,
			"dashboard API should reject missing auth",
		);

		const unauthorizedQaQc = await fetch(`${BASE_URL}/api/data/qaqc`, {
			redirect: "manual",
		});
		assert.equal(
			unauthorizedQaQc.status,
			401,
			"qaqc API should reject missing auth",
		);
		...
		assert.equal(
			...
			200,
			"session login should accept a valid API key",
		);
		const sessionCookieHeader = sessionResponse.headers.get("set-cookie");
		assert(
			sessionCookieHeader?.includes("knowledge_dashboard_session="),
			"session login should set an auth cookie",
		);
		...
		assert.equal(
			...
			200,
			"dashboard API should accept a valid session cookie",
		);
		const dashboardJson = await dashboardResponse.json();
		assert(
			Array.isArray(dashboardJson.github),
			"dashboard payload should include github data",
		);
		assert(
			Array.isArray(dashboardJson.notebooklm),
			"dashboard payload should include notebook data",
		);
		...
		assert.equal(
			qaqcResponse.status,
			200,
			"qaqc API should accept a valid session cookie",
		);
		const qaqcJson = await qaqcResponse.json();
		assert.equal(
			qaqcJson.verdict,
			"APPROVED",
			"qaqc payload should expose the smoke fixture verdict",
		);
		assert.equal(
			qaqcJson.total.passed,
			1,
			"qaqc payload should expose fixture totals",
		);
	} finally {
