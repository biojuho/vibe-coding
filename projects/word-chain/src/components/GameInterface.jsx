...
import { pickAiWord, validateMove } from "../utils/gameLogic";
...
const GameInterface = () => {
	...
	const [highScore, setHighScore] = useState(
		parseInt(localStorage.getItem("wordChainHighScore") || "0"),
	);
	...
	const handleSend = (e) => {
		...
		if (!result.ok) {
			setHistory((prev) => [
				...prev,
				{ text: `⚠️ ${result.reason}`, sender: "system" },
			]);
			return;
		}
		...
		>
			...
			<header className="p-4 border-b-2 border-cyber-blue/50 bg-black/40 backdrop-blur flex justify-between items-center sticky top-0 z-10">
				...
				<div className="text-right space-y-1">
					<Badge className="text-cyber-pink border-cyber-pink/30 bg-cyber-pink/10 font-mono text-sm">
				...
				<AnimatePresence>
					{history.map((msg, idx) => (
						...
						>
							...
							>
								{msg.sender === "com" && (
									<Cpu size={14} className="mb-1 inline-block mr-2" />
								)}
								{msg.text}
							</div>
						</Motion.div>
					...
					{turn === "com" && !gameOver && (
						...
						>
							<div className="text-cyber-blue text-sm animate-pulse">
								AI is thinking...
							</div>
						</Motion.div>
					)}
								...
								<CardContent className="space-y-4">
									...
									<div className="space-y-2">
										...
										{score >= highScore && score > 0 && (
											<div className="text-cyber-yellow text-sm font-bold animate-pulse">
												🏆 NEW HIGH SCORE!
											</div>
										)}
									</div>
			...
			<form
				onSubmit={handleSend}
				className="p-4 border-t-2 border-cyber-pink/50 bg-black/60 sticky bottom-0"
			>
