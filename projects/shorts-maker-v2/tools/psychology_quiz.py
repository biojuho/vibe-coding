...
class PsychologyQuizGenerator:
    ...
    def __init__(
        ...
        explanation: str,
        twist_text: str = "",
        cta_text: str = "다음 퀴즈도 도전해보세요!",
    ):
        self.question = question
        self.option_a = option_a
        self.option_b = option_b
        self.answer = answer.upper()
        self.stat_pct = stat_percent
        self.explanation = explanation
        self.twist_text = twist_text
    ...
    def _ph1(self, draw, t):
        ...
        if 1.5 <= t < 4.5:
            ...
            ea = -90 + 360 * prog
            if ea > -90:
                draw.arc(bb, -90, ea, fill=(*self.LAVENDER, 255), width=6)
            nt = str(num)
            nw = self._tw(nt, self.f_countdown)
            draw.text(
                (cx - nw // 2, cy - self._th(nt, self.f_countdown) // 2 - 4),
                nt,
                font=self.f_countdown,
                fill=(255, 255, 255, 255),
            )

    # ── Phase 2: Options Slide-in ──
    def _ph2(self, draw, t):
        cw = self.W - self.MARGIN * 2
        ch = 180
        # Option A: slide from left
        ap = self._eo(t / 0.8)
        ax = int(-cw + (self.MARGIN + cw) * ap)
        ay = 520
        self._opt_card(draw, ax, ay, cw, ch, "A", self.option_a, self.LAVENDER, int(255 * ap))
        # Option B: slide from right
        bp = self._eo((t - 0.3) / 0.8)
        bx = int(self.W - (self.W - self.MARGIN) * bp)
        by = 780
    ...
    def _opt_card(self, draw, x, y, w, h, letter, text, bcol, alpha):
        ...
        draw.rounded_rectangle(
            [(x, y), (x + w, y + h)], radius=20, fill=(45, 27, 51, int(alpha * 0.8)), outline=(*bcol, alpha), width=4
        )
        br = 28
        bcx = x + 50
        bcy = y + h // 2
        ...
        draw.text(
            (bcx - lw // 2, bcy - self._th(letter, self.f_badge) // 2 - 3),
            letter,
            font=self.f_badge,
    ...
    def _ph3(self, draw, overlay, t):
        ...
        final_y = 250
        cur_y = int(cor_y + (final_y - cor_y) * self._eo(t / 0.8))
    ...
    def _ph4(self, draw, t):
        ...
        if self._twist_lines:
            lh = self._th("가", self.f_body) + 16
            cw = self.W - self.MARGIN * 2
            ch_ = lh * len(self._twist_lines) + 50
            cy_ = 500
    ...
    def _render(self, t):
        ...
        if t < self.P1:
            ...
            self._ph2(d, t - self.P1)
        elif t < self.P3:
            self._ph3(d, ov, t - self.P2)
        else:
            self._ph4(d, t - self.P3)
...
def main():
    ...
    if not all([a.question, a.option_a, a.option_b, a.explanation]):
        ...
        return 1
    PsychologyQuizGenerator(a.question, a.option_a, a.option_b, a.answer, a.stat, a.explanation, a.twist).generate(
        a.out
    )
    return 0
