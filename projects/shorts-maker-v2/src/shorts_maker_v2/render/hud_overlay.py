    ...
    for fp in mono_candidates:
        ...
        if p.exists():
            try:
                ...
                logger.debug("hud_overlay: font candidate %s failed (%s), trying next", p, exc)
                continue
...
def render_hud_overlay(
    ...
    # 우상단
    draw.line(
        [(target_width - margin, margin), (target_width - margin - bracket_len, margin)],
        fill=color,
        width=bracket_width,
    )
    draw.line(
        [(target_width - margin, margin), (target_width - margin, margin + bracket_len)],
        fill=color,
        width=bracket_width,
    )
    # 좌하단
    draw.line(
        [(margin, target_height - margin), (margin + bracket_len, target_height - margin)],
        fill=accent,
        width=bracket_width,
    )
    draw.line(
        [(margin, target_height - margin), (margin, target_height - margin - bracket_len)],
        fill=accent,
        width=bracket_width,
    )
    # 우하단
    draw.line(
        [
            ...
            (target_width - margin - bracket_len, target_height - margin),
        ],
        fill=accent,
        width=bracket_width,
    )
    draw.line(
        [
            (target_width - margin, target_height - margin),
            (target_width - margin, target_height - margin - bracket_len),
        ],
        fill=accent,
        width=bracket_width,
    )
