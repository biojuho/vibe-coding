...
import { gameState, getElement, isDropping, shakeIntensity } from "./main.js";
...
export function renderCustomGraphics() {
	...
	const currentFruit = Matter.Composite.allBodies(engine.world).find(
		(b) => b.label === "current_fruit" && b.isStatic,
	);
...
export function createParticles(x, y, color) {
	...
	for (let i = 0; i < particleCount; i++) {
		const shapeType =
			Math.random() > 0.6 ? "circle" : Math.random() > 0.5 ? "square" : "star";
