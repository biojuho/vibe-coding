...
import { BOMB_RADIUS, FRUITS, GAME_HEIGHT, GAME_WIDTH } from "./config.js";
...
export function initPhysics(container, renderCustomGraphics) {
	...
	const wallOptions = {
		isStatic: true,
		render: { fillStyle: "#bcaaa4", visible: false },
	};
	const groundOptions = {
		isStatic: true,
		render: { fillStyle: "#8d6e63", visible: false },
	};
	...
	World.add(engine.world, [
		...
		Bodies.rectangle(
			GAME_WIDTH / 2,
			GAME_HEIGHT + WALL_THICKNESS / 2 - 60,
			GAME_WIDTH + 200,
			WALL_THICKNESS,
			groundOptions,
		), // Bottom
...
function applyMergeScore(baseScore, mergedFruitCount) {
	...
	const comboMultiplier =
		1 + Math.min(currentCombo - 1, difficulty.comboCap) * difficulty.comboStep;
	const clusterMultiplier =
		1 + Math.max(0, mergedFruitCount - 3) * difficulty.clusterStep;
...
function explodeBomb(x, y) {
	...
	const bombs = Composite.allBodies(engine.world).filter(
		(b) => b.plugin?.isBomb,
	);
	...
	const bodiesInRange = Query.region(
	...
	bodiesInRange.forEach((body) => {
		if (body.label === "fruit" && !body.isStatic) {
			...
			if (dist <= BOMB_EXPLOSION_RADIUS) {
				...
				destroyedCount++;
				if (Math.random() > 0.5)
					createParticles(body.position.x, body.position.y, "#333333");
			}
		}
	});
...
function decayComboIfNeeded() {
	...
	if (Date.now() - lastMergeAt <= getActiveDifficultyConfig().comboWindowMs)
		return;
	setComboCount(0);
	updateComboDisplay();
}
...
function checkGameOver() {
	...
	for (const fruit of fruits) {
		...
		if (fruit.position.y < 200) {
			if (
				Math.abs(fruit.velocity.y) < 0.2 &&
				Math.abs(fruit.velocity.x) < 0.2
			) {
...
export function createNewFruit(nextIsBomb) {
	...
	if (nextIsBomb) {
		...
		newFruit = Bodies.circle(GAME_WIDTH / 2, 80, fruitData.radius, {
			label: "current_fruit",
			render: { visible: false },
			plugin: { level: level, isBomb: false },
		});
	}
