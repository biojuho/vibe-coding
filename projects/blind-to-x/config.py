...
ERROR_FILTERED_EDITORIAL = "FILTERED_EDITORIAL"
    ...
    def validate(self) -> list[str]:
        ...
        for config_key, env_name in self._NOTION_ENV_KEYS.items():
            ...
            if config_val.startswith("${") and not env_val:
                warnings.append(f"'{config_key}'가 미치환 상태이며 env '{env_name}'도 미설정 → Notion 업로드 불가.")
            elif not config_val and not env_val:
                warnings.append(f"'{config_key}' 및 env '{env_name}' 모두 비어있음 → Notion 업로드 불가.")
        ...
        w_total = sum(
            float(self.get(f"ranking.weights.{k}", 0) or 0) for k in ("scrape_quality", "publishability", "performance")
        )
