...
class CostTracker:
    ...
    def _load_persisted_totals(self) -> None:
        ...
        try:
            for attempt in range(3):
                ...
                gemini_image_count = int(summary.get("gemini_image_count", 0) or 0)
                if attempt < 2 and total_usd <= 0 and gemini_image_count <= 0:
                    time.sleep(0.02)
                    continue

                self.current_cost = total_usd
                self.gemini_image_count = gemini_image_count
    ...
    def is_budget_exceeded(self) -> bool:
        ...
        if exceeded:
            _try_send_telegram(
                f"🚨 [blind-to-x] 일일 API 예산 초과!\n사용: ${self.current_cost:.3f} / 한도: ${self.daily_budget:.3f}",
                level="CRITICAL",
            )
        return exceeded
    ...
    def get_summary(self) -> str:
        ...
        for provider, calls in sorted(self.provider_calls.items()):
            ...
            provider_lines.append(f"- {provider}: {calls} calls (input={tokens['input']}, output={tokens['output']})")
