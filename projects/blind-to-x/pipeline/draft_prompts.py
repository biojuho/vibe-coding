    ...
    def _build_prompt(
        ...
        if "twitter" in output_formats:
            ...
            if draft_format == "thread" and "thread" in twitter_templates:
                ...
                # Hardcoded fallback (조용한 해설자 톤 — shorts 철학 적용)
                twitter_block = f"""\n[트위터(X) 초안 — 들려주듯 조용히]
...
3. 생략과 강조: 원문의 모든 내용을 담지 마세요. 가장 인상적인 지점 {empathy_anchor} 하나에만 집중하세요.
4. 마무리: 여운이 남는 한 줄로 조용히 끝내세요. 질문·CTA·"여러분 ~?"·"댓글로 ~" 모두 금지.
5. 형식: {self.max_length}자 이내 (가능하면 200자 안으로). 출처 '{source}' 표기는 선택 — 도입 강박 금지.

[기타]
- 본문에 링크나 해시태그는 넣지 마세요 (답글에 넣을 것).
- 1개의 안만 작성하세요. 추천 톤: '{recommended_draft_type}'.
...
"""
        ...
        selection_brief_lines = [
            ...
            f"파생각: {profile.get('spinoff_angle', '') or '현실 비교, 자기 경험'}",
            "",
            "[반드시 지킬 것]",
