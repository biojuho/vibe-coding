...
class NotionUploader(
    ...
    def __init__(self, config):
        ...
        self._env_props = {
            key: str(os.environ.get(env_name, "")).strip() for key, env_name in self.PROP_ENV_OVERRIDES.items()
        }
    ...
    async def _retrieve_collection(self):
        ...
        try:
            ...
            if not database.get("properties"):
                logger.warning(
                    "notion-client returned empty properties for database %s. Falling back to raw httpx call.",
                    self.database_id,
                )
                try:
                    import httpx
