"""Shared loader for split YAML rule files with legacy fallback support."""
...
LEGACY_RULES_FILE = PROJECT_ROOT / "classification_rules.yaml"
...
def has_split_rule_files() -> bool:
    return any((RULES_DIR / file_name).exists() for file_name in RULE_FILE_ORDER)
...
def load_rules(force_reload: bool = False) -> dict[str, Any]:
    ...
    if has_split_rule_files():
        merged: dict[str, Any] = {}
        for file_name in RULE_FILE_ORDER:
            merged.update(_read_yaml_file(RULES_DIR / file_name))
        _merged_rules_cache = merged
        logger.debug("Loaded split rules from %s", RULES_DIR)
        return copy.deepcopy(_merged_rules_cache)

    _merged_rules_cache = _read_yaml_file(LEGACY_RULES_FILE)
    if _merged_rules_cache:
        logger.debug("Loaded legacy rules from %s", LEGACY_RULES_FILE)
    return copy.deepcopy(_merged_rules_cache)
...
def get_rule_file_path(section: str) -> Path:
    ...
    if file_name and has_split_rule_files():
        return RULES_DIR / file_name
    return LEGACY_RULES_FILE


def write_legacy_rules_snapshot(target: Path | None = None) -> Path:
    if yaml is None:
        raise RuntimeError("PyYAML is required to write rule snapshots")

    snapshot_path = target or LEGACY_RULES_FILE
    snapshot_path.parent.mkdir(parents=True, exist_ok=True)
    merged_rules = load_rules(force_reload=True)
    header = (
        "# Autogenerated compatibility snapshot.\n# Edit files under rules/ and rebuild this snapshot when needed.\n\n"
    )
    body = yaml.safe_dump(merged_rules, allow_unicode=True, sort_keys=False, default_flow_style=False)
    snapshot_path.write_text(header + body, encoding="utf-8")
    logger.info("Wrote legacy rules snapshot to %s", snapshot_path)
