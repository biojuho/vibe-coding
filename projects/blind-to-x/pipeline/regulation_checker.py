...
from pipeline.rules_loader import get_rule_section, reset_rules_cache
...
def _load_regulations() -> dict:
    """Load platform regulations with a small local cache."""
    global _regulation_cache
    if _regulation_cache is not None:
        return _regulation_cache
    _regulation_cache = get_rule_section("platform_regulations", {})
    return _regulation_cache
...
class RegulationChecker:
    ...
    def build_regulation_context(self, platforms: list[str] | None = None) -> str:
        ...
        lines.append("※ 각 플랫폼 초안 작성 후 반드시 <regulation_check> 태그 안에 자체 검증 결과를 작성하세요.")
