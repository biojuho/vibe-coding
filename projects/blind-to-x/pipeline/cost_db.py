...
# Schema/migration constants and helpers live in cost_db_schema.py so the
# DDL surface is testable in isolation. The private aliases below preserve
# the historical import path used by older tests and call sites — `noqa`
...
class CostDatabase:
    ...
    def _init_db(self) -> None:
        # Schema DDL and idempotent ALTER TABLE migrations live in
        # `pipeline.cost_db_schema` — see its docstring. The list of
        # post-CREATE migration columns is `MIGRATION_COLUMNS` over there.
        with self._conn() as conn:
            _schema_init_db(conn)

    @staticmethod
    def _ensure_column(conn: sqlite3.Connection, table_name: str, column_name: str, ddl: str) -> None:
        _schema_ensure_column(conn, table_name, column_name, ddl)
    ...
    def get_daily_trend(self, days: int = 30) -> list[dict[str, Any]]:
        ...
        try:
            ...
            img_by_date = {r["date"]: (float(r["usd"] or 0), int(r["cnt"] or 0)) for r in image_rows}
            result = []
                ...
                try:
                    for table in tables:
                        ...
                        old_rows = src_conn.execute(
                            ...
                            (cutoff,),  # noqa: S608 — table from _ARCHIVE_TABLES frozenset
                        ).fetchall()
                        if not old_rows:
                            result[table] = 0
                            continue
