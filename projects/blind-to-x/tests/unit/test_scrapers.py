...
class TestAssessQuality:
    ...
    def test_score_clamped_to_100(self):
        ...
        post = {
            "title": "완벽한 직장인 게시글",
            "content": "연봉 이직 회사문화 복지 상사 이야기 직장인이라면 공감할 내용들 많이 있습니다 정말로요",
        }
        result = scraper.assess_quality(post)
...
class TestBrowserContextPool:
    ...
    @pytest.mark.asyncio
    async def test_release_with_none_ctx_does_not_enqueue(self):
        """ctx=None일 때 큐에 아무것도 넣지 않아야 함 (풀 오염 방지)."""
        from scrapers.browser_pool import BrowserContextPool
        ...
        pool._release(None)
        assert pool._queue.empty(), "ctx=None release must NOT enqueue anything"

    @pytest.mark.asyncio
    async def test_acquire_close_cycle_preserves_queue_size(self):
        """N회 acquire+close 사이클 후 큐 크기가 pool size와 같아야 함."""
        from scrapers.browser_pool import BrowserContextPool
        ...
        assert pool._queue.qsize() == pool.size

    @pytest.mark.asyncio
    async def test_release_queue_full_does_not_raise(self):
        """큐가 가득 찬 상태에서 _release 호출 시 예외 없이 경고만 발생."""
        from scrapers.browser_pool import BrowserContextPool
        ...
        pool._queue.put_nowait(ctx_a)
        pool._contexts = [ctx_a, ctx_b]
        pool._ready = True

        # 큐가 이미 가득 찬 상태에서 추가 release — 예외 없어야 함
        pool._release(ctx_b)
        assert pool._queue.qsize() == 1  # 초과 삽입되지 않음

    def test_pooled_page_proxies_attributes(self):
        """_PooledPage가 내부 page 속성을 투명하게 위임하는지 검증."""
        from scrapers.browser_pool import _PooledPage

        page_mock = MagicMock()
        page_mock.url = "https://example.com"
        pool_mock = MagicMock()

        proxy = _PooledPage(page_mock, pool_mock, ctx=MagicMock())
...
class TestUrlSchemeValidation:
    ...
    def test_utm_params_stripped(self):
        ...
        result = NotionUploader.canonicalize_url("https://teamblind.com/post/abc?utm_source=twitter&utm_medium=social")
...
class TestImageGeneratorFallback:
    ...
    async def test_gemini_success(self):
        ...
        with (
            ...
            result = await gen.generate_image(
                "A frustrated office worker sitting alone at a desk", topic_cluster="연봉", emotion_axis="분노"
            )
    ...
    async def test_gemini_none_falls_back_to_pollinations(self):
        ...
        with (
            ...
            result = await gen.generate_image(
                "A person contemplating career change in modern office", topic_cluster="이직", emotion_axis="허탈"
            )
    ...
    async def test_both_free_none_returns_fallback_when_dalle_disabled(self):
        ...
        with (
            ...
            result = await gen.generate_image(
                "Corporate team culture meeting with coworkers discussing issues",
                topic_cluster="회사문화",
                emotion_axis="공감",
            )
...
# ════════════════════════════════════════════════════════════════════════
# BaseScraper._new_page_cm 테스트
# ════════════════════════════════════════════════════════════════════════


class TestNewPageContextManager:
    @pytest.mark.asyncio
    async def test_new_page_cm_closes_on_exception(self):
        """블록 내 예외 발생 시에도 page.close()가 호출되어야 함."""
        from scrapers.base import BaseScraper
        ...
        with pytest.raises(ValueError, match="test error"):
            async with scraper._new_page_cm() as page:
    ...
    @pytest.mark.asyncio
    async def test_new_page_cm_closes_on_success(self):
        """정상 실행 후에도 page.close()가 호출되어야 함."""
        from scrapers.base import BaseScraper
