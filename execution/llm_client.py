...
Usage (CLI - 연결 테스트):
    python execution/llm_client.py test            # 전체 프로바이더 연결 테스트
    python execution/llm_client.py test --provider google  # 특정 프로바이더만
"""
...
def _cache_get(key: str, ttl_sec: int) -> str | None:
    ...
    try:
        with sqlite3.connect(str(_CACHE_DB_PATH)) as conn:
            row = conn.execute(
                "SELECT content, created_at FROM llm_cache WHERE key=?", (key,)
            ).fetchone()
    ...
    try:
        ...
        with sqlite3.connect(str(_CACHE_DB_PATH)) as conn:
            conn.execute(
                ...
                "(key TEXT PRIMARY KEY, content TEXT, created_at REAL)"
            )
            conn.execute(
            ...
            for attempt in range(1, self.max_retries + 1):
                try:
                    logger.info(
                        ...
                        provider, model, attempt, self.max_retries,
                    )
