...
DEFAULT_EXCLUDES = {
    ...
    "dist", "build", ".next", ".tmp", "vendor", "target",
...
def is_generated(source: str) -> bool:
    ...
    markers = ["generated by", "auto-generated", "@generated", "do not edit",
               "automatically generated", "this file is generated"]
...
class BaseAnalyzer:
    ...
    def _check_line_length(self, lines, fp, limit=120):
        ...
        for i, line in enumerate(lines, 1):
            if len(line) > limit:
                issues.append(Issue(
                    file=fp, line=i, column=limit,
                    category="readability", severity="low", rule_id="GEN001",
    ...
    def _check_file_length(self, lines, fp, limit=500):
        if len(lines) > limit:
            return [Issue(
                file=fp, line=1, column=0,
                category="readability", severity="low", rule_id="GEN005",
                message=f"File has {len(lines)} lines (>{limit}). Consider splitting.",
                code_snippet="", suggestion_hint="Split into smaller modules.",
            )]
        return []
    ...
    def _check_todo_fixme(self, lines, fp):
        ...
        for i, line in enumerate(lines, 1):
            ...
            if m:
                issues.append(Issue(
                    file=fp, line=i, column=m.start(),
                    category="readability", severity="medium", rule_id="GEN003",
    ...
    def _check_hardcoded_credentials(self, lines, fp):
        ...
        for i, line in enumerate(lines, 1):
            ...
            for pat in SECRET_PATTERNS:
                if pat.search(line):
                    issues.append(Issue(
                        file=fp, line=i, column=0,
                        category="security", severity="high", rule_id="GEN004",
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ImportFrom):
                for alias in node.names:
                    if alias.name == "*":
                        issues.append(Issue(
                            file=fp, line=node.lineno, column=0,
                            category="readability", severity="low", rule_id="PY001",
    ...
    def _check_bare_except(self, tree, fp):
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ExceptHandler) and node.type is None:
                issues.append(Issue(
                    file=fp, line=node.lineno, column=0,
                    category="security", severity="medium", rule_id="PY013",
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Call) and isinstance(node.func, ast.Name):
                if node.func.id in ("eval", "exec"):
                    issues.append(Issue(
                        file=fp, line=node.lineno, column=node.col_offset,
                        category="security", severity="high", rule_id="PY011",
            ...
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                for default in node.args.defaults + node.args.kw_defaults:
                    if default and isinstance(default, (ast.List, ast.Dict, ast.Set)):
                        issues.append(Issue(
                            file=fp, line=node.lineno, column=0,
                            category="performance", severity="medium", rule_id="PY008",
        ...
        for node in ast.walk(tree):
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                ...
                if not snake_re.match(name):
                    issues.append(Issue(
                        file=fp, line=node.lineno, column=0,
                        category="readability", severity="low", rule_id="PY003",
                ...
                if not pascal_re.match(node.name):
                    issues.append(Issue(
                        file=fp, line=node.lineno, column=0,
                        category="readability", severity="low", rule_id="PY004",
        ...
        for node in ast.walk(tree):
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                ...
                if length > limit:
                    issues.append(Issue(
                        file=fp, line=node.lineno, column=0,
                        category="readability", severity="medium", rule_id="PY002",
        ...
        for node in ast.walk(tree):
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                ...
                if cc > limit:
                    issues.append(Issue(
                        file=fp, line=node.lineno, column=0,
                        category="readability", severity="medium", rule_id="PY005",
    ...
    def _check_nesting_depth(self, source, lines, fp, limit=4):
        ...
        for i, line in enumerate(lines, 1):
            ...
            if depth > limit and not line.strip().startswith("#"):
                issues.append(Issue(
                    file=fp, line=i, column=0,
                    category="readability", severity="low", rule_id="PY006",
        ...
        for node in ast.walk(tree):
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                ...
                if args_all_missing and return_missing:
                    issues.append(Issue(
                        file=fp, line=node.lineno, column=0,
                        category="readability", severity="low", rule_id="PY007",
    ...
    def _check_assert_non_test(self, tree, fp):
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Assert):
                issues.append(Issue(
                    file=fp, line=node.lineno, column=0,
                    category="security", severity="low", rule_id="PY015",
    ...
    def _check_sql_injection(self, lines, fp):
        ...
        for i, line in enumerate(lines, 1):
            if sql_pattern.search(line):
                issues.append(Issue(
                    file=fp, line=i, column=0,
                    category="security", severity="high", rule_id="PY014",
                ...
                for child in ast.walk(node):
                    if isinstance(child, ast.AugAssign) and isinstance(child.op, ast.Add):
                        if isinstance(child.target, ast.Name):
                            ...
                            if is_string_op:
                                issues.append(Issue(
                                    file=fp, line=child.lineno, column=0,
                                    category="performance", severity="low", rule_id="PY009",
...
class JSAnalyzer(BaseAnalyzer):
    ...
    def _check_var_usage(self, lines, fp):
        ...
        for i, line in enumerate(lines, 1):
            ...
            if m and not _in_comment_or_string(line, m.start(), "//"):
                issues.append(Issue(
                    file=fp, line=i, column=0,
                    category="readability", severity="medium", rule_id="JS001",
    ...
    def _check_loose_equality(self, lines, fp):
        ...
        for i, line in enumerate(lines, 1):
            ...
            if m and not _in_comment_or_string(line, m.start(), "//"):
                issues.append(Issue(
                    file=fp, line=i, column=0,
                    category="performance", severity="low", rule_id="JS005",
    ...
    def _check_console_log(self, lines, fp):
        ...
        for i, line in enumerate(lines, 1):
            ...
            if m and not _in_comment_or_string(line, m.start(), "//"):
                issues.append(Issue(
                    file=fp, line=i, column=0,
                    category="readability", severity="low", rule_id="JS002",
    ...
    def _check_eval(self, lines, fp):
        ...
        for i, line in enumerate(lines, 1):
            ...
            if m and not _in_comment_or_string(line, m.start(), "//"):
                issues.append(Issue(
                    file=fp, line=i, column=0,
                    category="security", severity="high", rule_id="JS006",
    ...
    def _check_innerhtml(self, lines, fp):
        ...
        for i, line in enumerate(lines, 1):
            if pat.search(line):
                issues.append(Issue(
                    file=fp, line=i, column=0,
                    category="security", severity="high", rule_id="JS007",
    ...
    def _check_callback_nesting(self, lines, fp, limit=3):
        ...
        if max_depth > limit:
            issues.append(Issue(
                file=fp, line=max_line, column=0,
                category="readability", severity="medium", rule_id="JS003",
    ...
    def _check_document_write(self, lines, fp):
        ...
        for i, line in enumerate(lines, 1):
            if pat.search(line):
                issues.append(Issue(
                    file=fp, line=i, column=0,
                    category="security", severity="medium", rule_id="JS009",
    ...
    def _check_sql_injection(self, lines, fp):
        ...
        for i, line in enumerate(lines, 1):
            if pat.search(line):
                issues.append(Issue(
                    file=fp, line=i, column=0,
                    category="security", severity="medium", rule_id="JS008",
    ...
    def _check_any_type(self, lines, fp):
        ...
        for i, line in enumerate(lines, 1):
            ...
            if m and not _in_comment_or_string(line, m.start(), "//"):
                issues.append(Issue(
                    file=fp, line=i, column=0,
                    category="readability", severity="medium", rule_id="TS001",
    ...
    def _check_ts_ignore(self, lines, fp):
        ...
        for i, line in enumerate(lines, 1):
            if "@ts-ignore" in line or "@ts-nocheck" in line:
                issues.append(Issue(
                    file=fp, line=i, column=0,
                    category="readability", severity="low", rule_id="TS002",
...
class JavaAnalyzer(BaseAnalyzer):
    ...
    def _check_empty_catch(self, source, lines, fp):
        ...
        for m in pat.finditer(source):
            ...
            issues.append(Issue(
                file=fp, line=line_num, column=0,
                category="readability", severity="medium", rule_id="JV001",
        ...
        for i, line in enumerate(lines, 1):
            if loop_pat.search(line):
                ...
                if concat_pat.search(line):
                    issues.append(Issue(
                        file=fp, line=i, column=0,
                        category="performance", severity="medium", rule_id="JV003",
    ...
    def _check_raw_types(self, lines, fp):
        ...
        for i, line in enumerate(lines, 1):
            if pat.search(line) and "<" not in line:
                issues.append(Issue(
                    file=fp, line=i, column=0,
                    category="readability", severity="low", rule_id="JV004",
    ...
    def _check_system_exit(self, lines, fp):
        ...
        for i, line in enumerate(lines, 1):
            if pat.search(line):
                issues.append(Issue(
                    file=fp, line=i, column=0,
                    category="security", severity="medium", rule_id="JV007",
    ...
    def _check_sql_injection(self, lines, fp):
        ...
        for i, line in enumerate(lines, 1):
            if pat.search(line):
                issues.append(Issue(
                    file=fp, line=i, column=0,
                    category="security", severity="high", rule_id="JV006",
    ...
    def _check_sysout(self, lines, fp):
        ...
        for i, line in enumerate(lines, 1):
            if pat.search(line):
                issues.append(Issue(
                    file=fp, line=i, column=0,
                    category="readability", severity="low", rule_id="JV008",
    ...
    def _check_error_ignored(self, lines, fp):
        ...
        for i, line in enumerate(lines, 1):
            ...
            if err_discard.search(line) and not _in_comment_or_string(line, 0, "//"):
                issues.append(Issue(
                    file=fp, line=i, column=0,
                    category="readability", severity="high", rule_id="GO001",
    ...
    def _check_panic(self, lines, fp):
        ...
        for i, line in enumerate(lines, 1):
            ...
            if m and not _in_comment_or_string(line, m.start(), "//"):
                issues.append(Issue(
                    file=fp, line=i, column=0,
                    category="readability", severity="low", rule_id="GO006",
        ...
        for i, line in enumerate(lines, 1):
            ...
            if re.match(r"\bfor\b", stripped):
                ...
                if "defer " in stripped:
                    issues.append(Issue(
                        file=fp, line=i, column=0,
                        category="performance", severity="low", rule_id="GO005",
    ...
    def _check_sql_injection(self, lines, fp):
        ...
        for i, line in enumerate(lines, 1):
            if pat.search(line):
                issues.append(Issue(
                    file=fp, line=i, column=0,
                    category="security", severity="medium", rule_id="GO004",
        ...
        for i, line in enumerate(lines, 1):
            for func, (rule, msg) in unsafe.items():
                ...
                if m and not _in_comment_or_string(line, m.start(), "//"):
                    issues.append(Issue(
                        file=fp, line=i, column=0,
                        category="security", severity="high", rule_id=rule,
                        message=msg,
                        code_snippet=line.strip(),
    ...
    def _check_format_string(self, lines, fp):
        ...
        for i, line in enumerate(lines, 1):
            if pat.search(line):
                issues.append(Issue(
                    file=fp, line=i, column=0,
                    category="security", severity="medium", rule_id="CC004",
    ...
    def _check_system_call(self, lines, fp):
        ...
        for i, line in enumerate(lines, 1):
            ...
            if m and not _in_comment_or_string(line, m.start(), "//"):
                issues.append(Issue(
                    file=fp, line=i, column=0,
                    category="security", severity="high", rule_id="CC008",
    ...
    def to_markdown(report: AnalysisReport) -> str:
        ...
        for fr in report.files:
            ...
            if fr.metrics:
                m = fr.metrics
                lines.append(
                    f"Lines: {m.total_lines} | Functions: {m.function_count} | "
...
def main():
    ...
    parser.add_argument("--category", choices=["readability", "performance", "security"], default=None,
                        help="Filter by category (default: all)")
    parser.add_argument("--max-file-size", type=int, default=1_048_576,
