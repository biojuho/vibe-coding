...
class BridgePolicy:
    ...
    def from_env(cls) -> "BridgePolicy":
        ...
        allowed_terms = tuple(
            item.strip() for item in allowed_terms_raw.split(",") if item.strip()
        )
        json_key_exemptions_raw = os.getenv("LLM_BRIDGE_JSON_KEY_EXEMPTIONS", "")
        json_key_exemptions = tuple(
            item.strip() for item in json_key_exemptions_raw.split(",") if item.strip()
        )
        fallbacks_raw = os.getenv("LLM_BRIDGE_FALLBACKS", "")
        fallback_providers = tuple(
            item.strip().lower() for item in fallbacks_raw.split(",") if item.strip()
        ) or ("deepseek", "google", "openai")
