...
# 탭 구성
# ===========================================================================
tab_prod, tab_youtube, tab_roi, tab_hooks = st.tabs([
    "📦 생산 현황", "📈 YouTube 성과", "💰 ROI 분석", "🎣 훅 패턴"
])
        ...
        if ch_perf:
            ...
            for c in ch_perf:
                table_data.append({
                    ...
                    "CPV (원/조회)": f"${c['total_cost'] / max(c['total_views'], 1) * 1000:.2f}/1K" if c["total_views"] > 0 else "-",
                })
    ...
    if not ch_perf or all(c["total_views"] == 0 for c in ch_perf):
        ...
        if cpv_data:
            ...
            fig_cpv.add_trace(go.Bar(
                ...
                marker_color=[
                    COLORS["success"] if c["cpv_1k"] < 0.5
                    else COLORS["warning"] if c["cpv_1k"] < 2.0
                    else COLORS["danger"]
                    for c in cpv_sorted
                ],
...
with tab_hooks:
    ...
    if not hook_data:
        ...
        fig_hook.add_trace(go.Bar(
            ...
            marker_color=[
                COLORS["channels"][i % len(COLORS["channels"])]
                for i in range(len(hook_sorted))
            ],
