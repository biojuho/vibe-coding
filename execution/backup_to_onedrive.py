...
Usage:
    python execution/backup_to_onedrive.py             # 백업 실행
    python execution/backup_to_onedrive.py --dry-run   # 미리보기 (복사 안 함)
    python execution/backup_to_onedrive.py --status     # 백업 현황 확인
"""
...
def backup(dry_run: bool = False) -> dict:
    ...
    if not _ONEDRIVE.exists():
        raise RuntimeError(
            f"OneDrive 경로를 찾을 수 없습니다: {_ONEDRIVE}\n"
...
def get_status() -> dict:
    ...
    if _STATUS_FILE.exists():
        try:
            status.update(
                json.loads(_STATUS_FILE.read_text(encoding="utf-8"))
            )
        except Exception:
            pass
