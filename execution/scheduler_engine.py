...
Usage:
    python execution/scheduler_engine.py list
    python execution/scheduler_engine.py add --name "Daily Backup" --exec python --args "scripts/backup_data.py" --cron "0 9 * * *"
    python execution/scheduler_engine.py add --name "Legacy Cmd" --command "python scripts/backup_data.py" --cron "0 9 * * *"
    python execution/scheduler_engine.py run-due
    python execution/scheduler_engine.py logs --limit 50
    python execution/scheduler_engine.py kpis
"""
...
def _validate_migration_column(table_name: str, column_name: str, definition_sql: str) -> str:
    ...
    if definition_sql != expected_definition:
        raise ValueError(
            f"Unexpected definition for {table_name}.{column_name}: {definition_sql}"
        )
