...
Usage (CLI):
    python execution/error_analyzer.py analyze
    python execution/error_analyzer.py report
    python execution/error_analyzer.py report --send
"""
...
def analyze_recent_errors(days: int = 7) -> Dict[str, Any]:
    ...
    return {
        ...
        "top_modules": [
            {"module": m, "count": c} for m, c in module_counts.most_common(3)
        ],
        "period_days": days,
    }
...
def detect_recurring_patterns(min_occurrences: int = 3) -> List[Dict[str, Any]]:
    ...
    grouped: Dict[str, Dict[str, Any]] = defaultdict(
        lambda: {"count": 0, "last_seen": "", "modules": set()}
    )
...
def generate_weekly_report() -> str:
    ...
    if patterns:
        ...
        for p in patterns[:10]:
            ...
            rows.append(
                f"| {p['error_type']} | {p['count']} | {p['last_seen'][:16]} | {mods} |"
            )
        pattern_lines = (
            "| Category | Count | Last Seen | Modules |\n"
            "| --- | --- | --- | --- |\n"
            + "\n".join(rows)
        )
    else:
    ...
    if by_cat:
        cat_lines = "\n".join(
            f"  - {cat}: {cnt}" for cat, cnt in sorted(by_cat.items(), key=lambda x: -x[1])
        )
    else:
        cat_lines = "  (none)"
    ...
    if top_mods:
        top_mod_lines = "\n".join(
            f"  - {m['module']}: {m['count']}" for m in top_mods
        )
    else:
        top_mod_lines = "  (none)"
