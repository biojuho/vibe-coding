...
Usage (CLI):
    python execution/debug_history_db.py add --severity P1 --module youtube_uploader \\
        --symptom "업로드 무한 루프" --cause "재시도 제한 없음" --solution "MAX_RETRIES 추가"
    python execution/debug_history_db.py list [--severity P1] [--module xxx]
    python execution/debug_history_db.py search "무한 루프"
...
def register_pattern(
    ...
    try:
        existing = conn.execute(
            "SELECT id, times_seen FROM error_patterns WHERE pattern = ?", (pattern,)
        ).fetchone()
        if existing:
            conn.execute(
            ...
            conn.execute(
                """INSERT INTO error_patterns (pattern, error_type, first_check, tool, module_hint, last_seen_at)
                   VALUES (?, ?, ?, ?, ?, ?)""",
                (pattern, error_type, first_check, tool, module_hint,
                 datetime.now().isoformat(timespec="seconds")),
            )
        conn.commit()
    finally:
...
def list_patterns() -> List[Dict]:
    ...
    try:
        rows = conn.execute(
            "SELECT * FROM error_patterns ORDER BY times_seen DESC"
        ).fetchall()
        return [dict(row) for row in rows]
    finally:
        conn.close()
...
def seed_known_patterns() -> int:
    ...
    patterns = [
        ...
        ("DuplicateWidget", "StreamlitError", "위젯 key 중복", "st.session_state", "pages/"),
