...
def read_stream_with_timeout(stream, timeout_sec):
...
def run_diagnostics():
    ...
    for name, config in SERVERS.items():
        ...
        if missing_keys:
            ...
            # Optional: Some servers might still boot but fail at call time,
            # but we proceed with import/handshake to see if the interface is OK.
        ...
        # 4. Import & Dependencies Probe
        # Run a brief test to see if standard imports succeed inside this environment
        probe_code = f"import sys; sys.path.append(r'{server_path.parent}'); import server"
        try:
            proc = subprocess.run(
        ...
        # 5. Stdio JSON-RPC Handshake Test
        ...
        try:
            ...
            # Formulate the JSON-RPC initialization request
            init_req = {
                "jsonrpc": "2.0",
                "id": 1,
                "method": "initialize",
            ...
            # Send the request line with a newline
            req_str = json.dumps(init_req) + "\n"
            p.stdin.write(req_str.encode('utf-8'))
            p.stdin.flush()

            # Read first line from stdout (JSON-RPC response)
            resp_line = read_stream_with_timeout(p.stdout, timeout_sec=4)

            # Clean up the subprocess thoroughly
            p.terminate()
            try:
                p.wait(timeout=2)
            except subprocess.TimeoutExpired:
            ...
            if resp_line is None:
                ...
                # Let's see if it parses to valid JSON
                try:
                    resp_json = json.loads(resp_line)
                    if "jsonrpc" in resp_json and ("result" in resp_json or "error" in resp_json):
...
def save_report(results, passed_all):
    ...
    md.append(f"**Overall Verdict:** {'🟢 ALL SERVERS FUNCTIONAL' if passed_all else '🟡 SOME SERVERS FAILED/DEGRADED'}")
    md.append("")
    md.append("This report lists the status of local workspace MCP servers based on a deterministic Stdio JSON-RPC handshake validation.")
    md.append("")
    ...
    for name, s in results.items():
        ...
        md.append(f"| **{name}** | {emoji_for(s['syntax_check'])} | {emoji_for(s['import_check'])} | {emoji_for(s['handshake'])} | {emoji_for(s['env_check'])} | {status_emoji} |")
