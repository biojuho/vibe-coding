...
def self_test():
    ...
    try:
        ...
        if nbib_text and nbib_text.strip():
            ...
            if ris_content.strip() and bib_content.strip() and enw_content.strip():
                print(
                    f"  [OK] PubMed endpoint (RIS: {len(ris_content)}B, BibTeX: {len(bib_content)}B, ENW: {len(enw_content)}B)"
                )
            else:
    ...
    try:
        ...
        if ris_content.strip() and bib_content.strip() and enw_content.strip():
            print(
                f"  [OK] CrossRef endpoint (RIS: {len(ris_content)}B, BibTeX: {len(bib_content)}B, ENW: {len(enw_content)}B)"
            )
        else:
