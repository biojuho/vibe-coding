...
4. **`.ai/PROJECTS.md` 업데이트** — 프로젝트 상태, 현재 초점, 출력 품질 기준, 릴리스 경계가 바뀌었으면 갱신한다.
5. **`.ai/CONTEXT.md` 업데이트** — 진행 상황이 바뀌었거나, 새 지뢰밭(반복 실수/API 제약/타이밍 이슈)을 발견했으면 "Minefield" 섹션에 추가한다.
6. **`.ai/DECISIONS.md` 기록** — 새 아키텍처 결정(ADR)이 있으면 추가한다. 기존 결정은 임의 변경 금지.
7. **로테이터 실행** — HANDOFF/TASKS/SESSION_LOG가 커졌으면 커밋 전에 정리한다. 모두 멱등이라 반복 실행해도 안전하다:
...
8. **커밋** — `[ai-context]` 태그로 스테이징 후 커밋한다:

```bash
git commit -m "[ai-context] <세션 요약>"
```

9. **멀티툴 레이스 검증** — 병렬 도구의 `git add -A`가 다른 도구의 미커밋 변경을 흡수할 수 있다. 커밋 직후 확인한다:
...
- 산문 백틱 참조 파일: `.ai/HANDOFF.md`, `.ai/TASKS.md`, `.ai/SESSION_LOG.md`, `.ai/PROJECTS.md`, `.ai/CONTEXT.md`, `.ai/DECISIONS.md`.
