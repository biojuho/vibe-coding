...
def build_mapping(
    segments: list[Segment], args: argparse.Namespace
        ...
        mapping.append(
            {
                ...
                "suggested_insert_text": " ".join(
                    candidate.citation_marker for candidate in candidates[: args.citations_per_segment]
                ),
            }
        )
        if args.sleep:
...
def summarize_mapping(mapping: list[dict[str, Any]], references: list[Candidate], errors: list[dict[str, str]]) -> str:
    return f"segments={len(mapping)} references={len(references)} errors={len(errors)}"
...
def mapping_to_json(
    mapping: list[dict[str, Any]], references: list[Candidate], args: argparse.Namespace, errors: list[dict[str, str]]
) -> dict[str, Any]:
    return {
        "scope": args.scope,
        "from_year": args.from_year,
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Segment text and export strict Nature/CNS-family citations for EndNote or Zotero."
    )
    ...
    parser.add_argument(
        "--outdir", help="Optional directory for outputs. If omitted, uses the output file parent or current directory."
    )
    parser.add_argument(
        "--format",
        choices=EXPORT_FORMAT_CHOICES,
        help="Reference export format: enw, ris, or zotero-rdf. Inferred from --output-file when omitted.",
    )
    parser.add_argument(
        "--with-artifacts", action="store_true", help="Also generate JSON/TSV/report/HTML review artifacts."
    )
    parser.add_argument(
        "--rows", type=int, default=30, help="Crossref rows per segment before journal-scope filtering."
    )
    parser.add_argument("--per-segment", type=int, default=3, help="Maximum candidates to keep per segment.")
    parser.add_argument(
        "--citations-per-segment", type=int, default=2, help="Markers to include in suggested insert text."
    )
    parser.add_argument(
        "--segment-chars", type=int, default=700, help="Split paragraphs longer than this many characters."
    )
...
def main(argv: list[str]) -> int:
    ...
    if args.with_artifacts:
        outdir / name_base if name_base else outdir / "citation"
