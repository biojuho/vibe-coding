...
def plot_dataset(ax, title, sample1_arr, sample2_arr, max_freq_radius, line_colors, line_styles, legend_labels):
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
    ax.spines["left"].set_linewidth(2)
    ax.spines["bottom"].set_linewidth(2)
    ax.plot(
        np.arange(len(sample1_arr))[:max_freq_radius],
    ...
    ax.plot(
        np.arange(len(sample2_arr))[:max_freq_radius],
...
if __name__ == "__main__":
    ...
    for dataset_idx, dataset_name in enumerate(["Kvasir", "CVC-ClinicDB", "CVC-ColonDB", "ETIS"]):
        ...
        ax.plot(x_vals, mean, color=color, linewidth=1.5, label=f"{padded_name}" + r" $(N=100)$")
    ...
    ax.plot(
        x_vals, polyp_arr, color=polyp_color, linewidth=2.5, label=f"{fb_names[0].ljust(fb_max_len)}" + r" $(N=500)$"
    )
    ax.plot(
        x_vals,
        background_arr,
        ...
        linewidth=2.5,
        linestyle=":",
        label=f"{fb_names[1].ljust(fb_max_len)}" + r" $(N=500)$",
    )
    ax.axvline(2, color="0.6", linestyle="--", linewidth=1.2, label=r"Radius$=2$")
    x_left = ax.get_xlim()[0]
