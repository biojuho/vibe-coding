...
if __name__ == "__main__":
    ...
    for i in range(len(x_orig)):
        for j in range(i + 1, len(x_orig)):
            ...
            if prob > threshold:
                ax2.plot(
                    [x_orig[i], x_orig[j]], [z_orig[i], z_orig[j]], color="black", linewidth=2, alpha=prob * 2, zorder=1
                )

    # Plot the swiss roll points on top
    scatter = ax2.scatter(
        x_orig, z_orig, c=t_orig, cmap="viridis", s=100, alpha=0.5, edgecolors="white", linewidth=1, zorder=2
    )
    ...
    plt.savefig(
        "figures/diffusion_swiss_roll.png",
        dpi=300,
        bbox_inches="tight",
        facecolor="white",
        edgecolor="none",
