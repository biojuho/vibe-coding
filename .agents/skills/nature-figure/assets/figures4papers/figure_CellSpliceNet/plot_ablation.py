...
if __name__ == "__main__":
    ...
    for i in range(1, num_methods):
        ...
        ax.annotate(
            "", xy=(x_pos, current_value), xytext=(x_pos, baseline), arrowprops=dict(arrowstyle="->", color="red", lw=4)
        )

        # Add reduction text near the top (at baseline level)
        ax.text(
            x_pos - 0.3, baseline + 0.005, r"$-$" + f"{reduction:.2f}", ha="left", va="bottom", fontsize=24, color="red"
        )
