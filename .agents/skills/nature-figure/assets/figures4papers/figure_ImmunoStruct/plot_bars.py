...
from raw_data import data_ablation_Cancer, data_ablation_IEDB, data_comparison_Cancer, data_comparison_IEDB
...
if __name__ == "__main__":
    ...
    ax.bar(
        range(len(data_comparison_IEDB["mean"])),
    ...
    ax.barh(
        range(len(data_ablation_IEDB["mean"][:, 0])),
    ...
    ax.barh(
        range(len(data_ablation_IEDB["mean"][:, 1])),
    ...
    ax.barh(
        range(len(data_ablation_IEDB["mean"][:, 2])),
    ...
    ax = fig.add_subplot(1, 4, 1)
    ax.bar(
        range(len(data_comparison_Cancer["mean"])),
    ...
    ax.bar(
        ...
        label=["ImmunoStruct", "No Contrastive Learning", "No Contrastive Learning &\nNo Transfer Learning"],
    )
    handles, labels = ax.get_legend_handles_labels()
    ax.set_xticks([])
