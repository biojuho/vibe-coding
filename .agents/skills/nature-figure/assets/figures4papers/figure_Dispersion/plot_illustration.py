...
def draw_geodesic(
    ...
    if draw_arrow:
        ...
        arrow1 = FancyArrowPatch(
            (x[1], y[1]), (x[0], y[0]), arrowstyle="-|>", color=color, mutation_scale=arrow_scale, lw=0
        )
        arrow2 = FancyArrowPatch(
            (x[-2], y[-2]), (x[-1], y[-1]), arrowstyle="-|>", color=color, mutation_scale=arrow_scale, lw=0
        )
        ax.add_patch(arrow1)
        ax.add_patch(arrow2)
    return ax
...
def plot_decorrelation(ax):
    ...
    ax.imshow(
        img_s, cmap="gray", extent=[-image_scale, image_scale, -image_scale, image_scale], vmin=0, vmax=1, alpha=1
    )
...
def plot_l2_repel(ax):
    ...
    for i in range(num_points):
        ax.plot([Z[i, 0], 0], [Z[i, 1], 0], [Z[i, 2], 0], linestyle="--", color="black", linewidth=1, alpha=0.8)
    ...
    ax.legend(handles=[arrow_disp, arrow_norm], frameon=False, loc="lower center", fontsize=24, bbox_to_anchor=(0.5, 0))
