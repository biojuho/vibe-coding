...
def test_upload_pending_items_records_youtube_error(monkeypatch):
    ...
    monkeypatch.setattr(cdb, "update_job", lambda item_id, **kwargs: updates.append((item_id, kwargs)))
    monkeypatch.setattr(
        uploader, "upload_video", lambda **kwargs: (_ for _ in ()).throw(RuntimeError("quota exceeded"))
    )
...
def test_cli_upload(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        uploader,
        "upload_video",
        lambda **kw: {"video_id": "xyz", "youtube_url": "https://youtu.be/xyz", "status": "uploaded"},
    )
    monkeypatch.setattr(
        sys,
        "argv",
...
def test_cli_upload_pending(monkeypatch, capsys):
    ...
    monkeypatch.setattr(uploader, "upload_pending_items", lambda **kw: [{"item_id": 1, "status": "uploaded"}])
    monkeypatch.setattr(
        sys,
        "argv",
        [
...
def test_cli_status(monkeypatch, capsys):
    ...
    monkeypatch.setattr(uploader, "get_video_status", lambda vid: {"video_id": vid, "upload_status": "processed"})
    monkeypatch.setattr(
        sys,
        "argv",
        [
...
def test_upload_pending_items_empty(monkeypatch, tmp_path):
    ...
    try:
        with (
            _patch("execution.content_db.init_db"),
