...
Usage:
    python workspace/execution/daily_report.py [--date YYYY-MM-DD] [--format json|markdown]
"""
...
def _find_git_repos() -> List[Path]:
    ...
    for root, directory_names, _file_names in os.walk(str(WORKSPACE)):
        directory_names[:] = [
            directory_name for directory_name in directory_names if directory_name not in REPO_SCAN_EXCLUDED_DIRS
        ]
...
def collect_file_changes(target_date: date) -> Dict:
    ...
    for root, directory_names, file_names in os.walk(str(WORKSPACE)):
        directory_names[:] = [
            directory_name for directory_name in directory_names if directory_name not in FILE_SCAN_EXCLUDED_DIRS
        ]
