...
Usage:
    ...
    python workspace/execution/telegram_notifier.py send --message "hello"
    python workspace/execution/telegram_notifier.py daily-report --date 2026-02-28
"""
    ...
    *,
    ...
    if ok:
        ...
        payload["last_message_id"] = (
            (response_payload or {}).get("result", {}).get("message_id") if isinstance(response_payload, dict) else None
        )
    else:
...
def format_daily_report_message(report: Dict[str, Any], max_commits: int = 5) -> str:
    ...
    if summary.get("llm_bridge_calls", 0):
        ...
        if top_reason_codes:
            top = ", ".join(f"{item.get('reason_code', '?')}={item.get('count', 0)}" for item in top_reason_codes[:3])
...
def flush_digest(*, title: str = "Digest") -> Optional[Dict[str, Any]]:
    ...
    for item in queue[-20:]:  # 최대 20건
        prefix = {"CRITICAL": "🚨", "WARNING": "⚠️", "INFO": "ℹ️"}.get(item.get("level", "INFO").upper(), "ℹ️")
