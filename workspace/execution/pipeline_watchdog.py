...
Usage (CLI):
    python workspace/execution/pipeline_watchdog.py              # 전체 점검 + 알림
    python workspace/execution/pipeline_watchdog.py --json       # JSON 출력만
    python workspace/execution/pipeline_watchdog.py --no-notify  # 알림 없이 점검만
    python workspace/execution/pipeline_watchdog.py --daily      # 매일 요약 리포트 (성공해도 전송)
...
_ROOT = REPO_ROOT
_BTX_ROOT = resolve_project_dir("blind-to-x", required_paths=("pipeline",))
...
# ── Watchdog 결과 저장 ───────────────────────────────────
_WATCHDOG_HISTORY = TMP_ROOT / "watchdog_history.json"
_HEARTBEAT_FILE = TMP_ROOT / "watchdog_heartbeat.json"
...
class PipelineWatchdog:
    ...
    def check_btx_last_run(self, max_hours: int = 26) -> Dict[str, Any]:
        ...
        로그 파일 경로: projects/blind-to-x/.tmp/logs/scheduled_*.log
        Lock 파일: projects/blind-to-x/.tmp/.running.lock
        """
        ...
        if lock_file.exists():
            try:
                lock_age_sec = (datetime.now() - datetime.fromtimestamp(lock_file.stat().st_mtime)).total_seconds()
                if lock_age_sec > 3600:
                    result["status"] = STATUS_WARN
                    result["detail"] = f"Stale lock ({lock_age_sec / 3600:.1f}h old) — 프로세스가 멈췄을 수 있음"
                    self.alerts.append(f"⚠️ blind-to-x lock 파일이 {lock_age_sec / 3600:.1f}시간째 남아있습니다 (좀비?)")
        ...
        if age_hours <= max_hours:
            if has_failure:
                result["status"] = STATUS_WARN
                result["detail"] = f"Last run: {last_mod:%m/%d %H:%M} ({age_hours:.1f}h ago) — 일부 실패 있음"
                self.alerts.append(f"⚠️ blind-to-x 마지막 실행에 실패된 태스크가 있습니다 ({latest_log.name})")
            else:
                result["status"] = STATUS_OK
                result["detail"] = f"Last run: {last_mod:%m/%d %H:%M} ({age_hours:.1f}h ago) ✓"
        else:
            result["status"] = STATUS_FAIL
            result["detail"] = f"Stale! Last run: {last_mod:%m/%d %H:%M} ({age_hours:.1f}h ago)"
            self.alerts.append(f"🚨 blind-to-x가 {age_hours:.0f}시간 동안 실행되지 않았습니다!")
    ...
    def check_scheduler_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            rows = conn.execute("SELECT name, enabled, failure_count, last_run, next_run FROM tasks").fetchall()
            conn.close()
            ...
            result["detail"] = "; ".join(issues) if issues else f"{len(rows)} tasks, all healthy"
    ...
    def _write_heartbeat(overall: str) -> None:
        ...
        try:
            ...
            _HEARTBEAT_FILE.write_text(json.dumps(heartbeat, ensure_ascii=False), encoding="utf-8")
        except Exception as exc:
    ...
    def _save_history(self, report: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if _WATCHDOG_HISTORY.exists():
                history = json.loads(_WATCHDOG_HISTORY.read_text(encoding="utf-8"))
    ...
    def send_daily_summary(self, report: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            icons = {STATUS_OK: "✅", STATUS_WARN: "⚠️", STATUS_FAIL: "❌", STATUS_SKIP: "⏭️", STATUS_RUNNING: "🔄"}
...
if __name__ == "__main__":
    ...
    parser.add_argument("--check-alive", action="store_true", help="워치독 heartbeat 상태만 확인 (자동 재시작 체크용)")
