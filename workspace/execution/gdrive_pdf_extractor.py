...
사용 예:
    python workspace/execution/gdrive_pdf_extractor.py extract --file-id 1ABC... --output .tmp/output.txt
    python workspace/execution/gdrive_pdf_extractor.py list-folder --folder-id 1XYZ...
"""
...
def list_folder_files(folder_id: str | None = None, *, max_results: int = 50) -> list[dict]:
    ...
    result = service.files().list(q=query, pageSize=max_results, fields=fields, orderBy="modifiedTime desc").execute()
...
def list_new_files_since(
    ...
    *,
    ...
    result = service.files().list(q=query, pageSize=100, fields=fields, orderBy="modifiedTime desc").execute()
    files = result.get("files", [])
