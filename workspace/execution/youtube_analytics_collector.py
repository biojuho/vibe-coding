...
Usage:
    python workspace/execution/youtube_analytics_collector.py
    python workspace/execution/youtube_analytics_collector.py --channel "건강/과학"
"""
...
def _build_youtube_service():
    ...
    if not os.path.exists(_TOKEN_FILE):
        raise FileNotFoundError(f"OAuth 토큰 없음: {_TOKEN_FILE}\nyoutube_uploader.py의 인증 흐름을 먼저 실행하세요.")
    ...
    if not creds.valid:
        if creds.expired and creds.refresh_token:
            ...
            raise RuntimeError("OAuth 토큰 만료 및 refresh 불가. youtube_uploader.py로 재인증 필요.")
...
def collect_and_update(channel: str | None = None) -> dict[str, Any]:
    ...
    uploaded = [item for item in items if item.get("youtube_status") == "uploaded" and item.get("youtube_video_id")]
