...
Usage:
    python workspace/execution/youtube_uploader.py auth-check
    python workspace/execution/youtube_uploader.py upload --video /path/to/video.mp4 --title "제목"
    python workspace/execution/youtube_uploader.py upload-pending --limit 3
...
def get_auth_status() -> dict[str, Any]:
    ...
    try:
        ...
        status["ready"] = True
        status["reason"] = "토큰 준비 완료" if status["token_valid_or_refreshable"] else "refresh 필요 또는 재인증 필요"
        return status
    ...
    while response is None:
        try:
            ...
            if retry > MAX_RETRIES:
                raise RuntimeError(f"Upload failed after {MAX_RETRIES} retries: {chunk_err}") from chunk_err
