...
with worker_cols[2]:
    ...
    st.write("-" if ops_summary["seconds_since_heartbeat"] is None else str(ops_summary["seconds_since_heartbeat"]))
with worker_cols[3]:
...
task_filter = st.selectbox(
    ...
    format_func=lambda x: "All Tasks" if x is None else next((t.name for t in tasks if t.id == x), str(x)),
)
