...
실행: streamlit run workspace/execution/pages/result_dashboard.py
"""
...
with tab_detail:
    ...
    if not all_items:
        ...
        for item in filtered:
            ...
            with st.expander(
                f"{emoji} [{item['id']}] {item['title'][:50]} — {item.get('views', 0) or 0:,}회 조회",
                expanded=False,
            ):
                # 기본 정보
                info_cols = st.columns(3)
                info_cols[0].markdown(
                    f"**플랫폼**: {PLATFORMS.get(item['platform'], {}).get('display', item['platform'])}"
                )
                ...
                if auto_stats:
                    ...
                    with st.form(f"manual_stats_{item['id']}"):
                        sc = st.columns(3)
                        new_views = sc[0].number_input(
                            "조회수/노출", value=item.get("impressions", 0) or 0, min_value=0, key=f"v_{item['id']}"
                        )
                        new_likes = sc[1].number_input(
                            "좋아요", value=item.get("likes", 0) or 0, min_value=0, key=f"l_{item['id']}"
                        )
                        new_comments = sc[2].number_input(
                            "댓글", value=item.get("comments", 0) or 0, min_value=0, key=f"c_{item['id']}"
                        )

                        sc2 = st.columns(3)
                        new_rt = sc2[0].number_input(
                            "리트윗/공유", value=item.get("retweets", 0) or 0, min_value=0, key=f"rt_{item['id']}"
                        )
                        new_bm = sc2[1].number_input(
                            "북마크/저장", value=item.get("bookmarks", 0) or 0, min_value=0, key=f"bm_{item['id']}"
                        )
