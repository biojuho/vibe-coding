...
if not is_configured():
    st.warning("GitHub token not configured. Add `GITHUB_PERSONAL_ACCESS_TOKEN` to `infrastructure/.env`.")
    st.stop()
...
with col_heatmap:
    ...
    if heatmap:
        ...
        fig = go.Figure(go.Bar(x=dates, y=counts, marker_color="#7c3aed"))
        fig.update_layout(
            title="Daily Commits",
            xaxis_title="Date",
            yaxis_title="Commits",
