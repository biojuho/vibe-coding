...
Usage:
    python workspace/execution/notion_shorts_sync.py --all
    python workspace/execution/notion_shorts_sync.py --item-id 1
    python workspace/execution/notion_shorts_sync.py --since 2026-03-01
    python workspace/execution/notion_shorts_sync.py --channel "의학/건강"
    python workspace/execution/notion_shorts_sync.py --all --status success
"""
...
def _cli() -> None:
    ...
    if args.item_id:
        ...
        print(f"[Notion] 동기화 완료: 생성 {created} / 업데이트 {updated} / 오류 {errors} (총 {len(results)}건)")
        for r in results:
            if r["action"] == "error":
