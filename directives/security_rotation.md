...
```bash
# 수동 체크
python scripts/key_rotation_checker.py --dry-run

# Task Scheduler 등록 (매주 월요일 09:00)
# 작업: python scripts/key_rotation_checker.py
```
...
5. `.env.meta` 업데이트:
   ```bash
   python scripts/key_rotation_checker.py --update OPENAI_API_KEY
   ```
...
- [ ] `python scripts/key_rotation_checker.py --dry-run` — 경고 없음
- [ ] `python execution/api_usage_tracker.py check-keys` — 모든 키 Configured
