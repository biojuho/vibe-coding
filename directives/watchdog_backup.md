...
python execution/pipeline_watchdog.py

# JSON 출력 (스크립트 연동용)
python execution/pipeline_watchdog.py --json

# 점검만 (알림 없이)
python execution/pipeline_watchdog.py --no-notify

# 매일 요약 리포트 전송 (상태 무관)
python execution/pipeline_watchdog.py --daily
```

### 자동 실행
- `blind-to-x/run_scheduled.py`에 통합됨
...
### 백업 실행
```bash
python execution/backup_to_onedrive.py           # 실제 백업
python execution/backup_to_onedrive.py --dry-run  # 미리보기
python execution/backup_to_onedrive.py --status   # 현황 확인
```
...
3. `python execution/telegram_notifier.py check`으로 봇 상태 점검
