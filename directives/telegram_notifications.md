...
3. Run `python execution/telegram_notifier.py updates --limit 10` to inspect recent updates and discover the chat id.
4. Save the chat id in `.env` as `TELEGRAM_CHAT_ID`.
5. Run `python execution/telegram_notifier.py check` and `python execution/telegram_notifier.py send --message "test"` to validate the connection.
