...
## 개요
모든 프로젝트에서 7개 LLM 프로바이더를 자동 fallback 체인으로 사용합니다.
...
|-----------|---------|----------|------|------|
| Google Gemini | `GOOGLE_API_KEY` | gemini-2.5-flash | 무료 tier | 가장 우선 |
...
```bash
# 전체 프로바이더 연결 테스트
python execution/llm_client.py test

# 특정 프로바이더만 테스트
python execution/llm_client.py test --provider deepseek

# 활성화 상태 확인
python execution/llm_client.py status
```
