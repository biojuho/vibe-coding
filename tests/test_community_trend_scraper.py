...
def test_get_community_trends_fmkorea_only():
    ...
    mock_instance.scrape.return_value = [
        {"title": "FM 트렌드1", "source": "fmkorea", "views": "50000", "recommendations": "200", "link": "https://fmkorea.com/1"},
        {"title": "FM 트렌드2", "source": "fmkorea", "views": "30000", "recommendations": "100", "link": "https://fmkorea.com/2"},
    ]
...
def test_get_community_trends_ppomppu_only():
    ...
    mock_instance = MagicMock()
    mock_instance.scrape.return_value = [
        {"title": "뽐뿌 핫딜", "source": "ppomppu", "views": "10000", "recommendations": "50", "link": "https://ppomppu.co.kr/1"},
    ]
    mock_cls.return_value = mock_instance
...
def test_get_community_trends_both_sources():
    fm_cls = MagicMock()
    fm_cls.return_value.scrape.return_value = [{"title": "FM1", "source": "fmkorea", "views": "0", "recommendations": "0", "link": ""}]
    pp_cls = MagicMock()
    pp_cls.return_value.scrape.return_value = [{"title": "PP1", "source": "ppomppu", "views": "0", "recommendations": "0", "link": ""}]

    with patch.object(cts, "_FMKoreaScraper", fm_cls), \
         patch.object(cts, "_PpomppuScraper", pp_cls):
        result = cts.get_community_trends()
    assert len(result) == 2
...
def test_get_community_trends_limit():
    ...
    mock_cls.return_value.scrape.return_value = [
        {"title": f"T{i}", "source": "fmkorea", "views": "0", "recommendations": "0", "link": ""}
        for i in range(20)
    ]
