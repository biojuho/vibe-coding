...
class TestRun:
    ...
    def test_full_run(self, mock_pages, mock_stats, mock_update):
        ...
        mock_stats.return_value = {
            "abc123": {"views": 500, "likes": 50, "comments": 10}
        }
        ...
        mock_pages.return_value = [
            {
                ...
                "properties": {
                    ...
                    "YouTube Video ID": {
                        "rich_text": [{"plain_text": "existing_id"}]
                    },
                },
            }
        ]
...
class TestRunExtra:
    ...
    def test_v_param_url(self, mock_pages):
        mock_pages.return_value = [
            {"id": "p1", "properties": {"유튜브 URL": {"url": "https://youtube.com/watch?v=qwerty"}, "YouTube Video ID": {"rich_text": []}}}
        ]
        result = run(dry_run=True)
        assert result["skipped"] == 1
    ...
    def test_update_failure(self, mock_pages, mock_stats, mock_update):
        mock_pages.return_value = [
            {"id": "p1", "properties": {"유튜브 URL": {"url": "https://youtu.be/abc"}, "YouTube Video ID": {"rich_text": []}}}
        ]
