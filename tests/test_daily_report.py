...
def test_generate_report_includes_llm_bridge(monkeypatch):
    ...
    monkeypatch.setattr(dr, "collect_scheduler_logs", lambda _date: [{"task_name": "job", "started_at": _date.isoformat(), "exit_code": 0}])
    monkeypatch.setattr(
        dr,
    ...
    monkeypatch.setattr(dr, "_save_report", lambda report_date, report_payload: dr.REPORT_DIR / f"daily_{report_date.isoformat()}.json")
