...
def test_cli_item_id_prints_result(monkeypatch, capsys):
    _patch_env(monkeypatch)
    monkeypatch.setattr(
        sys, "argv", ["notion_shorts_sync.py", "--item-id", "42"],
    )
    monkeypatch.setattr(
        ns, "sync_item", lambda item_id: {"action": "created", "page_id": "page-abc", "error": ""},
    )
...
def test_cli_all_prints_summary(monkeypatch, capsys):
    _patch_env(monkeypatch)
    monkeypatch.setattr(
        sys, "argv", ["notion_shorts_sync.py", "--all", "--channel", "science", "--since", "2026-03-01", "--status", "success"],
    )
    monkeypatch.setattr(
        ns, "sync_all",
